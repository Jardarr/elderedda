const lokialtercation = [
	{
		id: 1,
		class: "m-7",
		contentON: (
			<span>
				<p className="mb-2">
					Ægir, er öðru nafni hét Gymir, hann hafði búit ásum öl, þá er hann
					hafði fengit ketil inn mikla, sem nú er sagt. Til þeirar veizlu kom
					Óðinn ok Frigg, kona hans. Þórr kom eigi, því at hann var í austrvegi.
					Sif var þar, kona Þórs, Bragi ok Iðunn, kona hans. Týr var þar. Hann
					var einhendr, — Fenrisúlfr sleit hönd af honum, þá er hann var
					bundinn. Þar var Njörðr ok kona hans Skaði, Freyr ok Freyja, Víðarr,
					son Óðins. Loki var þar ok þjónustumenn Freys, Byggvir ok Beyla. Margt
					var þar ása ok alfa. Ægir átti tvá þjónustumenn, Fimafengr ok Eldir.
					Þar var lýsigull haft fyrir elts ljós. Sjálft barst þar öl. Þar var
					griðastaðr mikill. Menn lofuðu mjök, hversu góðir þjónustumenn Ægis
					váru. Loki mátti eigi heyra þat, ok drap hann Fimafeng. Þá skóku æsir
					skjöldu sína ok æpðu at Loka ok eltu hann braut til skógar, en þeir
					fóru at drekka. Loki hvarf aftr ok hitti úti Eldi. Loki kvaddi hann:
				</p>
			</span>
		),

		contentRU: (
			<span>
				<p className="mb-2">
					Эгир, который иначе назывался Гюмир, наварил асам пива, когда получил
					огромный котел, о чем только что было рассказано. На этот пир пришли
					Один и Фригг, его жена. Тор не пришел, потому что он был в то время на
					востоке. Сив была там, жена Тора, Браги и Идун, жена его. Тюр был там,
					он был однорукий, Волк Фенрир откусил ему руку, когда Волка связывали.
					Там был Ньерд и жена его Скади, Фрейр и Фрейя, Видар, сын Одина. Локи
					был там и слуги Фрейра — Бюггвир и Бейла. Много там было асов и
					альвов. У Эгира было двое слуг — Фимафенг и Эльдир. Светящееся золото
					освещало палату. Пиво само лилось в кубки. Все должны были соблюдать
					там мир. Гости с большой похвалой говорили, какие у Эгира хорошие
					слуги. Локи не стерпел этого и убил Фимафенга. Тогда асы стали
					потрясать щитами и кричать на Локи. Они прогнали его в лес, а сами
					сели пировать.
					<p>Локи вернулся и встретил Эльдира. Локи обратился к нему:</p>
				</p>
			</span>
		),
	},
	{
		id: 2,
		number: "1.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				«Segðu þat, Eldir,
				<br />
				svá at þú einugi
				<br />
				feti gangir framar,
				<br />
				hvat hér inni
				<br />
				hafa at ölmálum
				<br />
				sigtíva synir.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				«Эльдир, ответь,
				<br />
				прежде чем ты
				<br />
				с места сойдешь:
				<br />
				о чем на пиру
				<br />
				за пивом хмельным
				<br />
				беседуют боги?»
			</span>
		),
	},
	{
		id: 3,
		number: "2.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Eldir kvað:
				<br />
				«Of vápn sín dæma
				<br />
				ok um vígrisni sína
				<br />
				sigtíva synir;
				<br />
				ása ok alfa
				<br />
				er hér inni eru,
				<br />
				manngi er þér í orði vinr.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Эльдир сказал:
				<br />
				«Об оружье своем,
				<br />
				о смелости в битвах
				<br />
				беседуют боги;
				<br />
				но никто из них другом
				<br />
				тебя не зовет —<br />
				ни асы, ни альвы».
			</span>
		),
	},
	{
		id: 4,
		number: "3.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Inn skal ganga
				<br />
				Ægis hallir í<br />
				á þat sumbl at sjá;
				<br />
				jöll ok áfu
				<br />
				færi ek ása sonum,
				<br />
				ok blend ek þeim svá meini mjöð.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«К Эгиру в дом —<br />
				войти я решил
				<br />
				и на пир посмотреть;
				<br />
				раздор и вражду
				<br />
				я им принесу,
				<br />
				разбавлю мед злобой».
			</span>
		),
	},
	{
		id: 5,
		number: "4.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Eldir kvað:
				<br />
				«Veiztu, ef þú inn gengr
				<br />
				Ægis hallir í<br />
				á þat sumbl at sjá,
				<br />
				hrópi ok rógi
				<br />
				ef þú eyss á holl regin,
				<br />á þér munu þau þerra þat.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Эльдир сказал:
				<br />
				«Если в палаты
				<br />
				войти ты решил,
				<br />
				на пир посмотреть
				<br />
				и асов забрызгать
				<br />
				грязной бранью —<br />
				об тебя ж оботрут ее».
			</span>
		),
	},
	{
		id: 6,
		number: "5.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Veiztu þat, Eldir,
				<br />
				ef vit einir skulum
				<br />
				sáryrðum sakask,
				<br />
				auðigr verða
				<br />
				mun ek í andsvörum,
				<br />
				ef þú mælir til margt.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Знаешь ли, Эльдир,
				<br />
				если начнем мы
				<br />
				обидно браниться,
				<br />
				я ответами буду
				<br />
				богаче тебя,
				<br />
				если ты не замолкнешь».
			</span>
		),
	},
	{
		id: 7,
		class: "m-7",
		contentON: (
			<span>
				<p className="mb-2">
					Síðan gekk Loki inn í höllina. En er þeir sá, er fyrir váru, hverr inn
					var kominn, þögnuðu þeir allir.
				</p>
			</span>
		),

		contentRU: (
			<span>
				<p>
					После этого Локи вошел в палату. Но когда сидевшие внутри увидели, кто
					вошел, они все замолкли. Локи сказал:
				</p>
			</span>
		),
	},
	{
		id: 8,
		number: "6.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þyrstr ek kom
				<br />
				þessar hallar til,
				<br />
				Loftr, um langan veg
				<br />
				ásu at biðja,
				<br />
				at mér einn gefi
				<br />
				mæran drykk mjaðar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				«Я, Лофт, издалека,
				<br />
				жаждой томимый,
				<br />
				в палату пришел,
				<br />
				асов прошу я,
				<br />
				чтоб кто-нибудь подал
				<br />
				мне доброго меда.
			</span>
		),
	},
	{
		id: 9,
		number: "7.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Hví þegið ér svá,
				<br />
				þrungin goð,
				<br />
				at þér mæla né meguð?
				<br />
				Sessa ok staði
				<br />
				velið mér sumbli at
				<br />
				eða heitið mik heðan.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Что ж вы молчите,
				<br />
				могучие боги,
				<br />
				что слова не скажете?
				<br />
				Пустите меня
				<br />
				на пиршество ваше
				<br />
				иль прочь прогоните!»
			</span>
		),
	},
	{
		id: 10,
		number: "8.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Bragi kvað:
				<br />
				«Sessa ok staði
				<br />
				velja þér sumbli at
				<br />
				æsir aldregi,
				<br />
				því at æsir vitu,
				<br />
				hveim þeir alda skulu
				<br />
				gambansumbl of geta.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Браги сказал:
				<br />
				«Не пустят тебя
				<br />
				на пиршество наше
				<br />
				боги могучие;
				<br />
				ибо ведомо им,
				<br />
				кого надлежит
				<br />
				на пир приглашать».
			</span>
		),
	},
	{
		id: 11,
		number: "9.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Mantu þat, Óðinn,
				<br />
				er vit í árdaga
				<br />
				blendum blóði saman?
				<br />
				Ölvi bergja
				<br />
				lézktu eigi mundu,
				<br />
				nema okkr væri báðum borit.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Один, когда-то —<br />
				помнишь ли? — кровь
				<br />
				мы смешали с тобою, —<br />
				сказал ты, что пива
				<br />
				пить не начнешь,
				<br />
				если мне не нальют».
			</span>
		),
	},
	{
		id: 12,
		number: "10.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Óðinn kvað:
				<br />
				«Rístu þá, Viðarr,
				<br />
				ok lát ulfs föður
				<br />
				sitja sumbli at,
				<br />
				síðr oss Loki
				<br />
				kveði lastastöfum
				<br />
				Ægis höllu í.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Один сказал:
				<br />
				«Видар, ты встань,
				<br />
				пусть Волка отец
				<br />
				сядет за стол наш,
				<br />
				чтоб Локи не начал
				<br />
				бранить нечестиво
				<br />
				гостей в доме Эгира».
			</span>
		),
	},
	{
		id: 13,
		class: "m-7",
		contentON: (
			<span>
				<p className="mb-2">
					Þá stóð Viðarr upp ok skenkði Loka. En áðr hann drykki, kvaddi hann
					ásuna:
				</p>
			</span>
		),

		contentRU: (
			<span>
				<p>
					Тогда Видар встал и налил кубок Локи, но тот, прежде чем выпить,
					обратился к асам:
				</p>
			</span>
		),
	},
	{
		id: 14,
		number: "11.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				«Heilir æsir,
				<br />
				heilar ásynjur
				<br />
				ok öll ginnheilög goð —<br />
				nema sá einn áss
				<br />
				er innar sitr,
				<br />
				Bragi, bekkjum á.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				«Славьтесь, асы,
				<br />
				и асиньи, славьтесь,
				<br />
				могучие боги!
				<br />
				Одного я не стану
				<br />
				приветствовать — Браги,
				<br />
				что сел в середине».
			</span>
		),
	},
	{
		id: 15,
		number: "12.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Bragi kvað:
				<br />
				«Mar ok mæki
				<br />
				gef ek þér míns féar,
				<br />
				ok bætir þér svá baugi Bragi,
				<br />
				síðr þú ásum
				<br />
				öfund of gjaldir,
				<br />
				grem þú eigi goð at þér.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Браги сказал:
				<br />
				«Меч и коня
				<br />
				тебе я вручу,
				<br />
				и кольцом откуплюсь я;
				<br />
				не начал бы только
				<br />
				ты ссор затевать;
				<br />
				бойся гнева богов!»
			</span>
		),
	},
	{
		id: 16,
		number: "13.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Jós ok armbauga
				<br />
				mundu æ vera
				<br />
				beggja vanr, Bragi;
				<br />
				ása ok alfa,
				<br />
				er hér inni eru,
				<br />
				þú ert við víg varastr
				<br />
				ok skjarrastr við skot.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Не дашь ты коня
				<br />
				и кольца ты не дашь:
				<br />
				посул твой напрасен;
				<br />
				из асов и альвов,
				<br />
				что здесь собрались,
				<br />
				ты самый трусливый
				<br />и схваток страшишься».
			</span>
		),
	},
	{
		id: 17,
		number: "14.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Bragi kvað:
				<br />
				«Veit ek, ef fyr útan værak,
				<br />
				svá sem fyr innan emk,
				<br />
				Ægis höll of kominn,
				<br />
				höfuð þitt
				<br />
				bæra ek í hendi mér;
				<br />
				lykak þér þat fyr lygi.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Браги сказал:
				<br />
				«Когда бы не здесь,
				<br />
				не у Эгира в доме
				<br />
				с тобою сошлись мы,
				<br />
				своею рукой
				<br />
				твою голову снял бы
				<br />в отплату за ложь».
			</span>
		),
	},
	{
		id: 18,
		number: "15.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Snjallr ertu í sessi,
				<br />
				skal-at-tu svá gera,
				<br />
				Bragi bekkskrautuðr;
				<br />
				vega þú gakk,
				<br />
				ef þú vreiðr séir;
				<br />
				hyggsk vætr hvatr fyrir.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Сидя ты храбр —<br />
				украшенье скамьи, —<br />
				но в битве беспомощен!
				<br />
				Смелость свою
				<br />
				покажи в сраженье!
				<br />
				Кто смел, тот не медлит».
			</span>
		),
	},
	{
		id: 19,
		number: "16.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Iðunn kvað:
				<br />
				«Bið ek þik, Bragi,
				<br />
				barna sifjar duga
				<br />
				ok allra óskmaga,
				<br />
				at þú Loka
				<br />
				kveðir-a lastastöfum
				<br />
				Ægis höllu í.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Идун сказала:
				<br />
				«Браги, не надо
				<br />
				У Эгира в доме
				<br />
				ссориться с Локи;
				<br />
				уместны ли распри
				<br />
				среди сыновей
				<br />
				родных и приемных!»
			</span>
		),
	},
	{
		id: 20,
		number: "17.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Iðunn,
				<br />
				þik kveð ek allra kvenna
				<br />
				vergjarnasta vera,
				<br />
				síztu arma þína
				<br />
				lagðir ítrþvegna
				<br />
				um þinn bróðurbana.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Идун, молчи!
				<br />
				До мужчин ты всех боле
				<br />
				из женщин жадна,
				<br />
				ведь руки твои
				<br />
				того обнимали,
				<br />
				кем брат твой убит».
			</span>
		),
	},
	{
		id: 21,
		number: "18.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Iðunn kvað:
				<br />
				«Loka ek kveðk-a
				<br />
				lastastöfum
				<br />
				Ægis höllu í:
				<br />
				Braga ek kyrri
				<br />
				bjórreifan;
				<br />
				vilk-at ek, at it vreiðir vegizk.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Идун сказала:
				<br />
				«Локи я словом
				<br />
				не оскорбляла
				<br />
				у Эгира в доме:
				<br />
				я Браги смирить
				<br />
				хмельного старалась
				<br />и распрю пресечь».
			</span>
		),
	},
	{
		id: 22,
		number: "19.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Gefjun kvað:
				<br />
				«Hví it æsir tveir
				<br />
				skuluð inni hér
				<br />
				sáryrðum sakask?
				<br />
				Loftki þat veit,
				<br />
				at hann leikinn er
				<br />
				ok hann fjörg öll fía.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Гевьон сказала:
				<br />
				«Зря вы, два аса,
				<br />
				друг друга язвите
				<br />
				речами бранчливыми:
				<br />
				ведает Лофт,
				<br />
				что слывет шутником
				<br />и любимцем богов».
			</span>
		),
	},
	{
		id: 23,
		number: "20.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Gefjun,
				<br />
				þess mun ek nú geta,
				<br />
				er þik glapði at geði
				<br />
				sveinn inn hvíti,
				<br />
				er þér sigli gaf
				<br />
				ok þú lagðir lær yfir.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Гевьон, молчи!
				<br />
				О юнце я напомню,
				<br />
				тебя совратившем:
				<br />
				дарил он уборы
				<br />
				в обмен на твои
				<br />
				любовные ласки».
			</span>
		),
	},
	{
		id: 24,
		number: "21.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Óðinn kvað:
				<br />
				«Ærr ertu, Loki,
				<br />
				ok örviti,
				<br />
				er þú fær þér Gefjun at gremi,
				<br />
				því at aldar örlög
				<br />
				hygg ek, at hon öll of viti
				<br />
				jafngörla sem ek.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Один сказал:
				<br />
				«Безумен ты, Локи,
				<br />
				что дерзостно вздумал
				<br />
				Гевьон гневить:
				<br />
				ведь ей, как и мне,
				<br />
				открыты и ясны
				<br />
				судьбы всех сущих».
			</span>
		),
	},
	{
		id: 25,
		number: "22.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Óðinn,
				<br />
				þú kunnir aldregi
				<br />
				deila víg með verum;
				<br />
				oft þú gaft,
				<br />
				þeim er þú gefa skyldir-a,
				<br />
				inum slævurum sigr.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Один, молчи!
				<br />
				Ты удачи в боях
				<br />
				не делил справедливо:
				<br />
				не воинам храбрым,
				<br />
				но трусам победу
				<br />
				нередко дарил ты».
			</span>
		),
	},
	{
		id: 26,
		number: "23.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Óðinn kvað:
				<br />
				«Veiztu, ef ek gaf,
				<br />
				þeim er ek gefa né skylda,
				<br />
				inum slævurum, sigr,
				<br />
				átta vetr
				<br />
				vartu fyr jörð neðan,
				<br />
				kýr mólkandi ok kona,
				<br />
				ok hefr þú þar börn borit,
				<br />
				ok hugða ek þat args aðal.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Один сказал:
				<br />
				«Коль не воинам храбрым,
				<br />
				но трусам победу
				<br />
				нередко дарил я,
				<br />
				то ты под землей
				<br />
				сидел восемь зим,
				<br />
				доил там коров,
				<br />
				рожал там детей,
				<br />
				ты — муж женовидный».
			</span>
		),
	},
	{
		id: 27,
		number: "24.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«En þik síða kóðu
				<br />
				Sámseyu í,
				<br />
				ok draptu á vétt sem völur;
				<br />
				vitka líki
				<br />
				fórtu verþjóð yfir,
				<br />
				ok hugða ek þat args aðal.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«А ты, я слышал,
				<br />
				на острове Самсей
				<br />
				бил в барабан,
				<br />
				средь людей колдовал,
				<br />
				как делают ведьмы, —<br />
				ты — муж женовидный».
			</span>
		),
	},
	{
		id: 28,
		number: "25.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Frigg kvað:
				<br />
				«Örlögum ykkrum
				<br />
				skylið aldregi
				<br />
				segja seggjum frá,
				<br />
				hvat it æsir tveir
				<br />
				drýgðuð í árdaga;
				<br />
				firrisk æ forn rök firar.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Фригг сказала:
				<br />
				«К чему говорить
				<br />
				о прежних делах,
				<br />
				о том, что свершали
				<br />
				вы, двое асов,
				<br />
				в давнее время;
				<br />
				что старое трогать!»
			</span>
		),
	},
	{
		id: 29,
		number: "26.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Frigg,
				<br />
				þú ert Fjörgyns mær
				<br />
				ok hefr æ vergjörn verit,
				<br />
				er þá Véa ok Vilja
				<br />
				léztu þér, Viðris kvæn,
				<br />
				báða i baðm of tekit.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Фригг, молчи!
				<br />
				Ты Фьергюна дочь
				<br />
				и нравом распутна:
				<br />
				хоть муж тебе Видрир,
				<br />
				ты Вили и Ве
				<br />
				обнимала обоих».
			</span>
		),
	},
	{
		id: 30,
		number: "27.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Frigg kvað:
				<br />
				«Veiztu, ef ek inni ættak
				<br />
				Ægis höllum i<br />
				Baldri líkan bur,
				<br />
				út þú né kvæmir
				<br />
				frá ása sonum,
				<br />
				ok væri þá at þér vreiðum vegit.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Фригг сказала:
				<br />
				«Будь тут со мной
				<br />
				у Эгира в доме
				<br />
				Бальдру подобный,
				<br />
				ты б не покинул
				<br />
				пиршество асов
				<br />
				без схватки жестокой».
			</span>
		),
	},
	{
		id: 31,
		number: "28.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Enn vill þú, Frigg,
				<br />
				at ek fleiri telja
				<br />
				mína meinstafi:
				<br />
				ek því réð,
				<br />
				er þú ríða sér-at
				<br />
				síðan Baldr at sölum.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Хочешь ты, Фригг,
				<br />
				дальше послушать
				<br />
				дерзкие речи:
				<br />
				из-за меня ведь
				<br />
				Бальдр не вернется
				<br />к тебе никогда».
			</span>
		),
	},
	{
		id: 32,
		number: "29.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Freyja kvað:
				<br />
				«Ærr ertu, Loki,
				<br />
				er þú yðra telr
				<br />
				ljóta leiðstafi;
				<br />
				örlög Frigg,
				<br />
				hygg ek, at öll viti,
				<br />
				þótt hon sjalfgi segi.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Фрейя сказала:
				<br />
				«Безумен ты, Локи,
				<br />
				зачем о злодействах
				<br />
				рассказ ты завел:
				<br />
				все судьбы Фригг,
				<br />
				я думаю, знает,
				<br />
				хоть в тайне хранит их».
			</span>
		),
	},
	{
		id: 33,
		number: "30.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Freyja,
				<br />
				þik kann ek fullgörva,
				<br />
				er-a þér vamma vant:
				<br />
				ása ok alfa,
				<br />
				er hér inni eru,
				<br />
				hverr hefir þinn hór verit.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Фрейя, молчи!
				<br />
				Тебя ль мне не знать;
				<br />
				ты тоже порочна:
				<br />
				всем ты любовь
				<br />
				свою отдавала —<br />
				всем асам и альвам».
			</span>
		),
	},
	{
		id: 34,
		number: "31.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Freyja kvað:
				<br />
				«Flá er þér tunga,
				<br />
				hygg ek, at þér fremr myni
				<br />
				ógótt of gala;
				<br />
				reiðir ro þér æsir
				<br />
				ok ásynjur,
				<br />
				hryggr muntu heim fara.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Фрейя сказала:
				<br />
				«Лжив твой язык;
				<br />
				тебя он, я знаю,
				<br />
				к беде приведет:
				<br />
				разгневаны асы
				<br />
				и асиньи тоже,
				<br />
				понурым вернешься ты».
			</span>
		),
	},
	{
		id: 35,
		number: "32.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Freyja,
				<br />
				þú ert fordæða
				<br />
				ok meini blandin mjök,
				<br />
				síz þik at bræðr þínum
				<br />
				stóðu blíð regin
				<br />
				ok myndir þú þá, Freyja, frata.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Фрейя, молчи!
				<br />
				Ты, злобная ведьма,
				<br />
				погрязла в разврате:
				<br />
				не тебе ли пришлось —<br />
				пойманной с братом —<br />
				визжать с перепугу!»
			</span>
		),
	},
	{
		id: 36,
		number: "33.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Njörðr kvað:
				<br />
				«Þat er válítit,
				<br />
				þótt sér varðir
				<br />
				vers fái, hós eða hvárs;
				<br />
				hitt er undr, er áss ragr
				<br />
				er hér inn of kominn
				<br />
				ok hefir sá börn of borit.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Ньерд сказал:
				<br />
				«Беды нет великой,
				<br />
				коль женщина делит
				<br />
				ложе с мужчиной,
				<br />
				хуже, что ас
				<br />
				женовидный, рожавший,
				<br />
				на пир наш пришел».
			</span>
		),
	},
	{
		id: 37,
		number: "34.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Njörðr,
				<br />
				þú vart austr heðan
				<br />
				gíls of sendr at goðum;
				<br />
				Hymis meyjar
				<br />
				höfðu þik at hlandtrogi
				<br />
				ok þér i munn migu.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Ньерд, молчи!
				<br />
				Не ты ли богами
				<br />
				был послан заложником;
				<br />
				дочери Хюмира
				<br />
				в рот твой мочились,
				<br />
				как будто в корыто».
			</span>
		),
	},
	{
		id: 38,
		number: "35.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Njörðr kvað:
				<br />
				«Sú erumk líkn,
				<br />
				er ek vark langt heðan
				<br />
				gísl of sendr at goðum,
				<br />
				þá ek mög gat,
				<br />
				þann er mangi fíár,
				<br />
				ok þykkir sá ása jaðarr.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Ньерд сказал:
				<br />
				«Пусть я далеко
				<br />
				заложником был,
				<br />
				но тем я утешен,
				<br />
				что сына родил я, —<br />
				дорог он всем,
				<br />
				он — первый из асов».
			</span>
		),
	},
	{
		id: 39,
		number: "36.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Hættu nú, Njörðr,
				<br />
				haf þú á hófi þik,
				<br />
				munk-a ek því leyna lengr:
				<br />
				við systur þinni
				<br />
				gaztu slíkan mög,
				<br />
				ok er-a þó vánu verr.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ньерд, перестань!
				<br />
				Похваляться не смей!
				<br />
				Не стану скрывать я:
				<br />
				прижил ты сына
				<br />
				с сестрою родной, —<br />
				что может быть хуже!»
			</span>
		),
	},
	{
		id: 40,
		number: "37.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Týr kvað:
				<br />
				«Freyr er beztr
				<br />
				allra ballriða
				<br />
				ása görðum í;
				<br />
				mey hann né grætir
				<br />
				né manns konu
				<br />
				ok leysir ór höftum hvern.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Тюр сказал:
				<br />
				«Фрейр самый лучший
				<br />
				в чертоге богов
				<br />
				воинственный всадник;
				<br />
				не обижал он
				<br />
				дев или жен,
				<br />
				отпускал полоненных».
			</span>
		),
	},
	{
		id: 41,
		number: "38.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Týr,
				<br />
				þú kunnir aldregi
				<br />
				bera tilt með tveim;
				<br />
				handar innar hægri
				<br />
				mun ek hinnar geta,
				<br />
				er þér sleit Fenrir frá.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Тюр, молчи!
				<br />
				Мирить не умел ты
				<br />
				в распре врагов:
				<br />
				правую руку
				<br />
				твою помяну я,
				<br />
				что Фенрир отгрыз».
			</span>
		),
	},
	{
		id: 42,
		number: "39.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Týr kvað:
				<br />
				«Handar em ek vanr,
				<br />
				en þú hróðrsvitnis,
				<br />
				böl er beggja þrá;
				<br />
				ulfgi hefir ok vel,
				<br />
				er í böndum skal
				<br />
				bíða ragnarökrs.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Тюр сказал:
				<br />
				«Я лишился руки,
				<br />
				а Хродрвитнир где твой!
				<br />
				Оба терпим потерю;
				<br />
				но тяжко и Волку
				<br />
				в цепях дожидаться
				<br />
				заката богов».
			</span>
		),
	},
	{
		id: 43,
		number: "40.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Týr,
				<br />
				þat varð þinni konu,
				<br />
				at hon átti mög við mér;
				<br />
				öln né penning
				<br />
				hafðir þú þess aldregi
				<br />
				vanréttis, vesall.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Тюр, молчи!
				<br />
				От меня родила
				<br />
				жена твоя сына;
				<br />
				за бесчестье с тобой
				<br />
				я не расчелся —<br />
				стерпел ты, презренный!»
			</span>
		),
	},
	{
		id: 44,
		number: "41.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Freyr kvað:
				<br />
				«Ulfr sé ek liggja
				<br />
				árósi fyrir,
				<br />
				unz rjúfask regin;
				<br />
				því mundu næst,
				<br />
				nema þú nú þegir,
				<br />
				bundinn, bölvasmiðr.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Фрейр сказал:
				<br />
				«Волк должен лежать
				<br />
				в устье реки
				<br />
				до кончины богов;
				<br />
				если ты не замолкнешь —<br />
				тотчас же будешь
				<br />
				закован, злодей!»
			</span>
		),
	},
	{
		id: 45,
		number: "42.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Gulli keypta
				<br />
				léztu Gymis dóttur
				<br />
				ok seldir þitt svá sverð;
				<br />
				en er Múspells synir
				<br />
				ríða Myrkvið yfir,
				<br />
				veizt-a þú þá, vesall, hvé þú vegr.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты золото отдал
				<br />
				за Гюмира дочь
				<br />
				и меч свой в придачу;
				<br />
				чем драться ты будешь,
				<br />
				коль Муспелля дети
				<br />
				сквозь Мюрквид поскачут?»
			</span>
		),
	},
	{
		id: 46,
		number: "43.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Byggvir kvað:
				<br />
				«Veiztu, ef ek eðli ættak
				<br />
				sem Ingunar-Freyr
				<br />
				ok svá sælligt setr,
				<br />
				mergi smæra
				<br />
				mölða ek þá meinkráku
				<br />
				ok lemða alla í liðu.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Бюггвир сказал:
				<br />
				«Был бы я равен
				<br />
				Ингунар-Фрейру
				<br />
				в чертоге счастливом,
				<br />
				я б растерзал,
				<br />
				разорвал бы я в клочья
				<br />
				ворону зловредную».
			</span>
		),
	},
	{
		id: 47,
		number: "44.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Hvat er þat it litla
				<br />
				er ek þat löggra sék
				<br />
				ok snapvíst snapir?
				<br />
				At eyrum Freys
				<br />
				mundu æ vera
				<br />
				ok und kvernum klaka.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Что там за мелочь
				<br />
				виляет хвостом,
				<br />
				пресмыкаясь пред сильными?
				<br />
				Вечно подачек
				<br />
				ты просишь у Фрейра,
				<br />
				за жерновом ноя».
			</span>
		),
	},
	{
		id: 48,
		number: "45.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Byggvir kvað:
				<br />
				«Byggvir ek heiti,
				<br />
				en mik bráðan kveða
				<br />
				goð öll ok gumar;
				<br />
				því em ek hér hróðugr,
				<br />
				at drekka Hrofts megir
				<br />
				allir öl saman.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Бюггвир сказал:
				<br />
				«Бюггвир зовусь,
				<br />
				меж людей и богов
				<br />
				быстрым прослыл я;
				<br />
				почетно сидеть мне
				<br />
				с сынами Хрофта
				<br />
				на пиршестве пышном».
			</span>
		),
	},
	{
		id: 49,
		number: "46.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Byggvir,
				<br />
				þú kunnir aldregi
				<br />
				deila með mönnum mat,
				<br />
				ok þik í flets strái
				<br />
				finna né máttu,
				<br />
				þá er vágu verar.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Бюггвир, молчи!
				<br />
				Не умел никогда ты
				<br />
				пищу подать;
				<br />
				не ты ль под столами
				<br />
				в соломе скрывался
				<br />
				при каждом сраженье!»
			</span>
		),
	},
	{
		id: 50,
		number: "47.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Heimdallr kvað:
				<br />
				«Ölr ertu, Loki,
				<br />
				svá at þú ert örviti,
				<br />
				— hví né lezk-a-ðu, Loki? —<br />
				því at ofdrykkja
				<br />
				veldr alda hveim,
				<br />
				er sína mælgi né man-at.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Хеймдалль сказал:
				<br />
				«Ты, Локи, от пива
				<br />
				рассудка лишился;
				<br />
				замолкнешь ли, Локи?
				<br />
				Язык свой не в силах
				<br />
				тот обуздать,
				<br />
				кто не в меру напьется».
			</span>
		),
	},
	{
		id: 51,
		number: "48.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Heimdallr,
				<br />
				þér var í árdaga
				<br />
				it ljóta líf of lagit;
				<br />
				örgu baki
				<br />
				þú munt æ vera
				<br />
				ok vaka vörðr goða.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Хеймдалль, молчи!
				<br />
				От начала времен
				<br />
				удел твой нелегок:
				<br />
				с мокрой спиной
				<br />
				на страже богов
				<br />
				неустанно стоишь ты».
			</span>
		),
	},
	{
		id: 52,
		number: "49.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Skaði kvað:
				<br />
				«Létt er þér, Loki;
				<br />
				mun-at-tu lengi svá
				<br />
				leika lausum hala,
				<br />
				því at þik á hjörvi
				<br />
				skulu ins hrímkalda magar
				<br />
				görnum binda goð.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Скади сказала:
				<br />
				«Локи, ты весел,
				<br />
				но будешь недолго
				<br />
				резвиться на воле,
				<br />
				ибо к скале тебя
				<br />
				сына кишками
				<br />
				боги привяжут».
			</span>
		),
	},
	{
		id: 53,
		number: "50.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Veiztu, ef mik á hjörvi
				<br />
				skulu ins hrímkalda magar
				<br />
				görnum binda goð,
				<br />
				fyrstr ok efstr
				<br />
				var ek at fjörlagi,
				<br />
				þars vér á Þjaza þrifum.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Если к скале меня
				<br />
				сына кишками
				<br />
				боги привяжут —<br />
				знай, что я первым
				<br />
				был и последним
				<br />в час гибели Тьяци».
			</span>
		),
	},
	{
		id: 54,
		number: "51.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Skaði kvað:
				<br />
				«Veiztu, ef fyrstr ok efstr
				<br />
				vartu at fjörlagi,
				<br />
				þá er ér á Þjaza þrifuð,
				<br />
				frá mínum véum
				<br />
				ok vöngum skulu
				<br />
				þér æ köld ráð koma.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Скади сказала:
				<br />
				«Если ты первым
				<br />
				был и последним
				<br />
				в час гибели Тьяци,
				<br />
				то в доме моем
				<br />
				всегда тебе будут
				<br />
				гибель готовить».
			</span>
		),
	},
	{
		id: 55,
		number: "52.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Léttari í málum
				<br />
				vartu við Laufeyjar son,
				<br />
				þá er þú létz mér á beð þinn boðit;
				<br />
				getit verðr oss slíks,
				<br />
				ef vér görva skulum
				<br />
				telja vömmin vár.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ласковей ты
				<br />
				призывала когда-то
				<br />
				Локи на ложе:
				<br />
				стоит то вспомнить,
				<br />
				коль начали счет мы
				<br />
				деяний недобрых».
			</span>
		),
	},
	{
		id: 56,
		class: "m-7",
		contentON: (
			<span>
				<p className="mb-2">
					þá gekk Sif fram ok byrlaði Loka í hrímkálki mjöð ok mælti:
				</p>
			</span>
		),

		contentRU: (
			<span>
				<p>
					Тогда вышла вперед Сив, налила Локи в хрустальный кубок меду и
					сказала:
				</p>
			</span>
		),
	},
	{
		id: 57,
		number: "53.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				«Heill ver þú nú, Loki,
				<br />
				ok tak við hrímkálki
				<br />
				fullum forns mjaðar,
				<br />
				heldr þú hana eina
				<br />
				látir með ása sonum
				<br />
				vammalausa vera.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				«Привет тебе, Локи!
				<br />
				Кубок хрустальный
				<br />
				с медом прими
				<br />
				и меня на пиру
				<br />
				могучих богов
				<br />в речах не порочь!»
			</span>
		),
	},
	{
		id: 58,
		class: "m-7",
		contentON: (
			<span>
				<p className="mb-2">Hann tók við horni ok drakk af:</p>
			</span>
		),

		contentRU: (
			<span>
				<p>Он взял рог и выпил.</p>
			</span>
		),
	},
	{
		id: 59,
		number: "54.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				«Ein þú værir,
				<br />
				ef þú svá værir,
				<br />
				vör ok gröm at veri;
				<br />
				einn ek veit,
				<br />
				svá at ek vita þykkjumk,
				<br />
				hór ok af Hlórriða,
				<br />
				ok var þat sá inn lævísi Loki.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				«Порочить не стал бы,
				<br />
				когда б ты и впрямь
				<br />
				была неприступной;
				<br />
				но знаю, с одним —<br />
				и мне ли не знать! —<br />
				изменила ты мужу, —<br />
				то был злобный Локи».
			</span>
		),
	},
	{
		id: 60,
		number: "55.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Beyla kvað:
				<br />
				«Fjöll öll skjalfa;
				<br />
				hygg ek á för vera
				<br />
				heiman Hlórriða;
				<br />
				han ræðr ró,
				<br />
				þeim er rægir hér
				<br />
				goð öll ok guma.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Бейла сказала:
				<br />
				«Горы дрожат,
				<br />
				то едет, я думаю,
				<br />
				Хлорриди грозный;
				<br />
				принудит молчать он
				<br />
				того, кто поносит
				<br />
				могучих богов».
			</span>
		),
	},
	{
		id: 61,
		number: "56.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Þegi þú, Beyla,
				<br />
				þú ert Byggvis kvæn
				<br />
				ok meini blandinn mjök,
				<br />
				ókynjan meira
				<br />
				kom-a med ása sonum;
				<br />
				öll ertu, deigja, dritin.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Ты, Бейла, молчи!
				<br />
				Ты, жена Бюггвира,
				<br />
				мрази вместилище;
				<br />
				выродок ты
				<br />
				меж богами великими,
				<br />
				скотница грязная!»
			</span>
		),
	},
	{
		id: 62,
		number: "57.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þá kom Þórr at ok kvað:
				<br />
				«Þegi þú, rög vættr,
				<br />
				þér skal minn þrúðhamarr,
				<br />
				Mjöllnir, mál fyrnema;
				<br />
				herðaklett
				<br />
				drep ek þér halsi af,
				<br />
				ok verðr þá þínu fjörvi of farit.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Тут вошел Тор и сказал:
				<br />
				«Мерзостный, смолкни!
				<br />
				Принудит к молчанью
				<br />
				тебя молот Мьелльнир!
				<br />
				Скалу твоих плеч
				<br />
				с плеч я снесу, —<br />
				конец твой настанет».
			</span>
		),
	},
	{
		id: 63,
		number: "58.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Jarðar burr
				<br />
				er hér nú inn kominn,
				<br />
				hví þrasir þú svá, Þórr?
				<br />
				En þá þorir þú ekki,
				<br />
				er þú skalt við ulfinn vega,
				<br />
				ok svelgr hann allan Sigföður.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Вот и сын ерд
				<br />
				прибыл сюда:
				<br />
				что ж браниться ты начал?
				<br />
				Не будешь ты смелым,
				<br />
				с Волком сражаясь,
				<br />
				что Одина сгубит».
			</span>
		),
	},
	{
		id: 64,
		number: "59.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þórr kvað:
				<br />
				«Þegi þú, rög vættr,
				<br />
				þér skal minn þrúðhamarr,
				<br />
				Mjöllnir, mál fyrnema;
				<br />
				upp ek þér verp
				<br />
				ok á austrvega,
				<br />
				síðan þik manngi sér.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Тор сказал:
				<br />
				«Мерзостный, смолкни!
				<br />
				Принудит к молчанью
				<br />
				тебя молот Мьелльнир!
				<br />
				Вверх я заброшу
				<br />
				тебя на восток, —<br />
				сгинешь совсем ты».
			</span>
		),
	},
	{
		id: 65,
		number: "60.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Austrförum þínum
				<br />
				skaltu aldregi
				<br />
				segja seggjum frá,
				<br />
				síz í hanska þumlungi
				<br />
				hnúkðir þú einheri,
				<br />
				ok þóttisk-a þú þá Þórr vera.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Полно тебе
				<br />
				поминать о походах
				<br />
				твоих на восток, —<br />
				ты в рукавице
				<br />
				прятался там,
				<br />
				не опомнясь от страха».
			</span>
		),
	},
	{
		id: 66,
		number: "61.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þórr kvað:
				<br />
				«Þegi þú, rög vættr,
				<br />
				þér skal minn þrúðhamarr,
				<br />
				Mjöllnir, mál fyrnema;
				<br />
				hendi inni hægri
				<br />
				drep ek þik Hrungnis bana,
				<br />
				svá at þér brotnar beina hvat.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Тор сказал:
				<br />
				«Мерзостный, смолкни!
				<br />
				Принудит к молчанью
				<br />
				тебя молот Мьелльнир!
				<br />
				Правой рукой
				<br />
				на тебя я обрушу
				<br />
				Хрунгнира гибель».
			</span>
		),
	},
	{
		id: 67,
		number: "62.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Lifa ætla ek mér
				<br />
				langan aldr,
				<br />
				þóttú hætir hamri mér;
				<br />
				skarpar álar
				<br />
				þóttu þér Skrýmis vera,
				<br />
				ok máttir-a þú þá nesti ná,
				<br />
				ok svalzt þú þá hungri heill.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Еще доведется
				<br />
				долго мне жить,
				<br />
				угроз не страшусь я;
				<br />
				Скрюмира были
				<br />
				крепки ремни,
				<br />
				до еды не достать —<br />
				от голода гиб ты».
			</span>
		),
	},
	{
		id: 68,
		number: "63.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þórr kvað:
				<br />
				«Þegi þú, rög vættr,
				<br />
				þér skal minn þrúðhamarr,
				<br />
				Mjöllnir, mál fyrnema;
				<br />
				Hrungnis bani
				<br />
				mun þér í hel koma
				<br />
				fyr nágrindr neðan.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Тор сказал:
				<br />
				«Мерзостный, смолкни!
				<br />
				Принудит к молчанью
				<br />
				тебя молот Мьелльнир!
				<br />
				Хрунгнира гибель
				<br />
				швырнет тебя в Хель
				<br />к воротам смерти».
			</span>
		),
	},
	{
		id: 69,
		number: "64.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Loki kvað:
				<br />
				«Kvað ek fyr ásum,
				<br />
				kvað ek fyr ása sonum,
				<br />
				þats mik hvatti hugr,
				<br />
				en fyr þér einum
				<br />
				mun ek út ganga,
				<br />
				því at ek veit, at þú vegr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Локи сказал:
				<br />
				«Я высказал асам,
				<br />
				я высказал асиньям
				<br />
				все, не таясь,
				<br />
				тебе ж уступлю
				<br />
				и отсюда уйду, —<br />
				ты станешь сражаться.
			</span>
		),
	},
	{
		id: 70,
		number: "65.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Öl gerðir þú, Ægir,
				<br />
				en þú aldri munt
				<br />
				síðan sumbl of gera;
				<br />
				eiga þín öll,
				<br />
				er hér inni er,
				<br />
				leiki yfir logi,
				<br />
				ok brenni þér á baki.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Пива ты, Эгир,
				<br />
				немало припас,
				<br />
				но напрасно старался:
				<br />
				пусть все, чем владеешь,
				<br />
				в пламени сгинет,
				<br />
				пусть опалит
				<br />
				огонь тебе спину!»
			</span>
		),
	},
	{
		id: 71,
		class: "m-7",
		contentON: (
			<span>
				<p className="mb-2">
					En eftir þetta falst Loki í Fránangrsforsi í lax líki. Þar tóku æsir
					hann. Hann var bundinn með þörmum sonar síns, Vála, en Narfi, sonr
					hans, varð at vargi. Skaði tók eitrorm ok festi upp yfir annlit Loka.
					Draup þar ór eitr. Sigyn, kona Loka, sat þar ok helt munnlaug undir
					eitrið. En er munnlaugin var full, bar hon út eitrið, en meðan draup
					eitrit á Loka. Þá kippðist hann svá hart við, at þaðan af skalf jörð
					öll. Þat eru nú kallaðir landsskjálftar.
				</p>
			</span>
		),

		contentRU: (
			<span>
				<p>
					После этого Локи, в образе лосося, спрятался в водопаде фьорда
					Франангр. Там асы поймали его. Он был связан кишками сына своего Нари,
					а сын его Нарви превратился в волка. Скади взяла ядовитую змею и
					повесила ее над лицом Локи. Из змеи капал яд. Сигюн, жена Локи, сидела
					там и подставляла чашу под капающий яд. А когда чаша наполнялась, она
					ее выносила, и в это время яд из змеи капал на Локи. Тогда он корчился
					так сильно, что вся земля дрожала. Теперь это называется
					землетрясением.
				</p>
			</span>
		),
	},
];

export { lokialtercation };
