const highsspeeches = [
	{
		id: 1,
		number: "1.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="w-40">
				<span className="text-2xl/4" style={{ fontFamily: "Eileen, sans-serif" }}>
                    G
                </span>
				áttir allar,
				<br />
				áðr gangi fram,
				<br />
				um skoðask skyli,
				<br />
				um skyggnast skyli,
				<br />
				því at óvíst er at vita,
				<br />
				hvar óvinir
				<br />
				sitja á fleti fyrir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Прежде чем в дом
				<br />
				войдешь, все входы
				<br />
				ты осмотри,
				<br />
				ты огляди, —<br />
				ибо как знать,
				<br />
				в этом жилище
				<br />
				недругов нет ли.
			</span>
		),
	},
	{
		id: 2,
		number: "2.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Gefendr heilir!
				<br />
				Gestr er inn kominn,
				<br />
				hvar skal sitja sjá?
				<br />
				Mjök er bráðr,
				<br />
				sá er á bröndum skal
				<br />
				síns of freista frama.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Дающим привет!
				<br />
				Гость появился!
				<br />
				Где место найдет он?
				<br />
				Торопится тот,
				<br />
				кто хотел бы скорей
				<br />у огня отогреться.
			</span>
		),
	},
	{
		id: 3,
		number: "3.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Elds er þörf,
				<br />
				þeims inn er kominn
				<br />
				ok á kné kalinn;
				<br />
				matar ok váða
				<br />
				er manni þörf,
				<br />
				þeim er hefr um fjall farit.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Дорог огонь
				<br />
				тому, кто с дороги,
				<br />
				чьи застыли колени;
				<br />
				в еде и одежде
				<br />
				нуждается странник
				<br />в горных краях.
			</span>
		),
	},
	{
		id: 4,
		number: "4.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Vatns er þörf,
				<br />
				þeim er til verðar kemr,
				<br />
				þerru ok þjóðlaðar,
				<br />
				góðs of æðis,
				<br />
				ef sér geta mætti,
				<br />
				orðs ok endrþögu.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Гостю вода
				<br />
				нужна и ручник,
				<br />
				приглашенье учтивое,
				<br />
				надо приветливо
				<br />
				речь повести
				<br />и выслушать гостя.
			</span>
		),
	},
	{
		id: 5,
		number: "5.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Vits er þörf,
				<br />
				þeim er víða ratar;
				<br />
				dælt er heima hvat;
				<br />
				at augabragði verðr,
				<br />
				sá er ekki kann
				<br />
				ok með snotrum sitr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Ум надобен тем,
				<br />
				кто далеко забрел, —<br />
				дома все тебе ведомо;
				<br />
				насмешливо будут
				<br />
				глядеть на невежду,
				<br />
				средь мудрых сидящего.
			</span>
		),
	},
	{
		id: 6,
		number: "6.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				At hyggjandi sinni
				<br />
				skyli-t maðr hræsinn vera,
				<br />
				heldr gætinn at geði;
				<br />
				þá er horskr ok þögull
				<br />
				kemr heimisgarða til,
				<br />
				sjaldan verðr víti vörum,
				<br />
				því at óbrigðra vin
				<br />
				fær maðr aldregi
				<br />
				en mannvit mikit.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Умом пред людьми
				<br />
				похваляться не надо —<br />
				скрывать его стоит;
				<br />
				если мудрец
				<br />
				будет молчать —<br />
				не грозит ему горе,
				<br />
				ибо нет на земле
				<br />
				надежнее друга,
				<br />
				чем мудрость житейская.
			</span>
		),
	},
	{
		id: 7,
		number: "7.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Inn vari gestr,
				<br />
				er til verðar kemr,
				<br />
				þunnu hljóði þegir,
				<br />
				eyrum hlýðir,
				<br />
				en augum skoðar;
				<br />
				svá nýsisk fróðra hverr fyrir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Гость осторожный,
				<br />
				дом посетивший,
				<br />
				безмолвно внимает —<br />
				чутко слушать
				<br />
				и зорко смотреть
				<br />
				мудрый стремится.
			</span>
		),
	},
	{
		id: 8,
		number: "8.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Hinn er sæll,
				<br />
				er sér of getr
				<br />
				lof ok líknstafi;
				<br />
				ódælla er við þat,
				<br />
				er maðr eiga skal
				<br />
				annars brjóstum í.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Счастливы те,
				<br />
				кто заслужил
				<br />
				похвалу и приязнь;
				<br />
				труднее найти
				<br />
				добрый совет
				<br />в груди у других.
			</span>
		),
	},
	{
		id: 9,
		number: "9.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Sá er sæll,
				<br />
				er sjalfr of á<br />
				lof ok vit, meðan lifir;
				<br />
				því at ill ráð
				<br />
				hefr maðr oft þegit
				<br />
				annars brjóstum ór.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Счастливы те,
				<br />
				кто в жизни славны
				<br />
				разумом добрым;
				<br />
				неладный совет
				<br />
				часто найдешь
				<br />у другого в груди.
			</span>
		),
	},
	{
		id: 10,
		number: "10.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Byrði betri
				<br />
				berr-at maðr brautu at
				<br />
				en sé mannvit mikit;
				<br />
				auði betra
				<br />
				þykkir þat í ókunnum stað;
				<br />
				slíkt er válaðs vera.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Нету в пути
				<br />
				драгоценней ноши,
				<br />
				чем мудрость житейская,
				<br />
				дороже сокровищ
				<br />
				она на чужбине —<br />
				то бедных богатство.
			</span>
		),
	},
	{
		id: 11,
		number: "11.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Byrði betri
				<br />
				berr-at maðr brautu at
				<br />
				en sé mannvit mikit;
				<br />
				vegnest verra
				<br />
				vegr-a hann velli at
				<br />
				en sé ofdrykkja öls.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Нету в пути
				<br />
				драгоценней ноши,
				<br />
				чем мудрость житейская,
				<br />
				хуже нельзя
				<br />
				в путь запастись,
				<br />
				чем пивом опиться.
			</span>
		),
	},
	{
		id: 12,
		number: "12.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Er-a svá gótt
				<br />
				sem gótt kveða
				<br />
				öl alda sona,
				<br />
				því at færa veit,
				<br />
				er fleira drekkr
				<br />
				síns til geðs gumi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Меньше от пива
				<br />
				пользы бывает,
				<br />
				чем думают многие;
				<br />
				чем больше ты пьешь,
				<br />
				тем меньше покорен
				<br />
				твой разум тебе.
			</span>
		),
	},
	{
		id: 13,
		number: "13.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Óminnishegri heitir
				<br />
				sá er yfir ölðrum þrumir,
				<br />
				hann stelr geði guma;
				<br />
				þess fugls fjöðrum
				<br />
				ek fjötraðr vark
				<br />í garði Gunnlaðar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Цапля забвенья
				<br />
				вьется над миром,
				<br />
				рассудок крадет;
				<br />
				крылья той птицы
				<br />
				меня приковали
				<br />в доме у Гуннлед.
			</span>
		),
	},
	{
		id: 14,
		number: "14.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ölr ek varð,
				<br />
				varð ofrölvi
				<br />
				at ins fróða Fjalars;
				<br />
				því er ölðr bazt,
				<br />
				at aftr of heimtir
				<br />
				hverr sitt geð gumi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Пьяным я был,
				<br />
				слишком напился
				<br />
				у мудрого Фьялара;
				<br />
				но лучшее в пиве —<br />
				что хмель от него
				<br />
				исчезает бесследно.
			</span>
		),
	},
	{
		id: 15,
		number: "15.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þagalt ok hugalt
				<br />
				skyli þjóðans barn
				<br />
				ok vígdjarft vera;
				<br />
				glaðr ok reifr
				<br />
				skyli gumna hverr,
				<br />
				unz sinn bíðr bana.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Осторожным быть должен
				<br />
				конунга отпрыск
				<br />
				и смелым в сраженье;
				<br />
				каждый да будет
				<br />
				весел и добр
				<br />
				до часа кончины.
			</span>
		),
	},
	{
		id: 16,
		number: "16.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ósnjallr maðr
				<br />
				hyggsk munu ey lifa,
				<br />
				ef hann við víg varask;
				<br />
				en elli gefr
				<br />
				hánum engi frið,
				<br />
				þótt hánum geirar gefi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Глупый надеется
				<br />
				смерти не встретить,
				<br />
				коль битв избегает;
				<br />
				но старость настанет —<br />
				никто от нее
				<br />
				не сыщет защиты.
			</span>
		),
	},
	{
		id: 17,
		number: "17.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Kópir afglapi
				<br />
				er til kynnis kemr,
				<br />
				þylsk hann um eða þrumir;
				<br />
				allt er senn,
				<br />
				ef hann sylg of getr,
				<br />
				uppi er þá geð guma.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Глазеет глупец,
				<br />
				приехавший в гости,
				<br />
				болтая иль молча;
				<br />
				а выпьет глоток —<br />
				и сразу покажет,
				<br />
				как мало в нем мудрости,
			</span>
		),
	},
	{
		id: 18,
		number: "18.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Sá einn veit
				<br />
				er víða ratar
				<br />
				ok hefr fjölð of farit,
				<br />
				hverju geði
				<br />
				stýrir gumna hverr,
				<br />
				sá er vitandi er vits.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знает лишь тот,
				<br />
				кто много земель
				<br />
				объездил и видел, —<br />
				коль сам он умен, —<br />
				что на уме
				<br />у каждого мужа.
			</span>
		),
	},
	{
		id: 19,
		number: "19.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Haldi-t maðr á keri,
				<br />
				drekki þó at hófi mjöð,
				<br />
				mæli þarft eða þegi,
				<br />
				ókynnis þess
				<br />
				vár þik engi maðr,
				<br />
				at þú gangir snemma at sofa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Пей на пиру,
				<br />
				но меру блюди
				<br />
				и дельно беседуй;
				<br />
				не прослывешь
				<br />
				меж людей неучтивым,
				<br />
				коль спать рано ляжешь.
			</span>
		),
	},
	{
		id: 20,
		number: "20.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Gráðugr halr,
				<br />
				nema geðs viti,
				<br />
				etr sér aldrtrega;
				<br />
				oft fær hlægis,
				<br />
				er með horskum kemr,
				<br />
				manni heimskum magi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Без толку жадный
				<br />
				старается жрать
				<br />
				себе на погибель;
				<br />
				смеются порой
				<br />
				над утробой глупца
				<br />
				на пиршестве мудрых.
			</span>
		),
	},
	{
		id: 21,
		number: "21.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Hjarðir þat vitu,
				<br />
				nær þær heim skulu,
				<br />
				ok ganga þá af grasi;
				<br />
				en ósviðr maðr
				<br />
				kann ævagi
				<br />
				síns of mál maga.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знают стада,
				<br />
				что срок наступил
				<br />
				покинуть им пастбища;
				<br />
				а кто неумен,
				<br />
				меры не знает,
				<br />
				живот набивая.
			</span>
		),
	},
	{
		id: 22,
		number: "22.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Vesall maðr
				<br />
				ok illa skapi
				<br />
				hlær at hvívetna;
				<br />
				hittki hann veit,
				<br />
				er hann vita þyrfti,
				<br />
				at hann er-a vamma vanr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Кто нравом тяжел,
				<br />
				тот всех осуждает,
				<br />
				смеется над всем;
				<br />
				ему невдомек,
				<br />
				а должен бы знать,
				<br />
				что сам он с изъяном.
			</span>
		),
	},
	{
		id: 23,
		number: "23.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ósviðr maðr
				<br />
				vakir um allar nætr
				<br />
				ok hyggr at hvívetna;
				<br />
				þá er móðr,
				<br />
				er at morgni kemr,
				<br />
				allt er víl sem var.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Глупый не спит
				<br />
				всю ночь напролет
				<br />
				в думах докучных;
				<br />
				утро настанет —<br />
				где же усталому
				<br />
				мудро размыслить.
			</span>
		),
	},
	{
		id: 24,
		number: "24.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ósnotr maðr
				<br />
				hyggr sér alla vera
				<br />
				viðhlæjendr vini;
				<br />
				hittki hann fiðr,
				<br />
				þótt þeir um hann fár lesi,
				<br />
				ef hann með snotrum sitr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Муж неразумный
				<br />
				увидит приязнь
				<br />
				в улыбке другого;
				<br />
				с мудрыми сидя,
				<br />
				глупец не поймет
				<br />
				над собою насмешки.
			</span>
		),
	},
	{
		id: 25,
		number: "25.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ósnotr maðr
				<br />
				hyggr sér alla vera
				<br />
				viðhlæjendr vini;
				<br />
				þá þat finnr,
				<br />
				er at þingi kemr,
				<br />
				at hann á formælendr fáa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Муж неразумный
				<br />
				увидит приязнь
				<br />
				в улыбке другого;
				<br />
				а после на тинге
				<br />
				едва ли отыщет
				<br />
				сторонников верных.
			</span>
		),
	},
	{
		id: 26,
		number: "26.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ósnotr maðr
				<br />
				þykkisk allt vita,
				<br />
				ef hann á sér í vá veru;
				<br />
				hittki hann veit,
				<br />
				hvat hann skal við kveða,
				<br />
				ef hans freista firar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Муж неразумный
				<br />
				все знает на свете,
				<br />
				в углу своем сидя;
				<br />
				но не найдет он
				<br />
				достойных ответов
				<br />в дельной беседе.
			</span>
		),
	},
	{
		id: 27,
		number: "27.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ósnotr maðr,
				<br />
				er með aldir kemr,
				<br />
				þat er bazt, at hann þegi;
				<br />
				engi þat veit,
				<br />
				at hann ekki kann,
				<br />
				nema hann mæli til margt;
				<br />
				veit-a maðr,
				<br />
				hinn er vettki veit,
				<br />
				þótt hann mæli til margt.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Муж неразумный
				<br />
				на сборище людном
				<br />
				молчал бы уж лучше;
				<br />
				не распознать
				<br />
				в человеке невежду,
				<br />
				коль он не болтлив,
				<br />
				но невежда всегда
				<br />
				не видит того,
				<br />
				что болтлив он безмерно.
			</span>
		),
	},
	{
		id: 28,
		number: "28.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Fróðr sá þykkisk,
				<br />
				er fregna kann
				<br />
				ok segja it sama;
				<br />
				eyvitu leyna
				<br />
				megu ýta synir,
				<br />
				því er gengr um guma.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Мудрым слывет,
				<br />
				кто расспросит других
				<br />
				и расскажет разумно;
				<br />
				скрыть не умеют
				<br />
				люди в беседах,
				<br />
				что с ними случилось.
			</span>
		),
	},
	{
		id: 29,
		number: "29.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ærna mælir,
				<br />
				sá er æva þegir,
				<br />
				staðlausu stafi;
				<br />
				hraðmælt tunga,
				<br />
				nema haldendr eigi,
				<br />
				oft sér ógótt of gelr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Кто молчать не умеет,
				<br />
				тот лишние речи
				<br />
				заводит нередко;
				<br />
				быстрый язык
				<br />
				накличет беду,
				<br />
				коль его не сдержать.
			</span>
		),
	},
	{
		id: 30,
		number: "30.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				At augabragði
				<br />
				skal-a maðr annan hafa,
				<br />
				þótt til kynnis komi;
				<br />
				margr þá fróðr þykkisk,
				<br />
				ef hann freginn er-at
				<br />
				ok nái hann þurrfjallr þruma.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Насмешливых взглядов
				<br />
				не надо бросать
				<br />
				на гостей приглашенных
				<br />
				не спросишь иного —<br />
				он мнит, что разумен,
				<br />и мирно пирует.
			</span>
		),
	},
	{
		id: 31,
		number: "31.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Fróðr þykkisk,
				<br />
				sá er flótta tekr,
				<br />
				gestr at gest hæðinn;
				<br />
				veit-a görla,
				<br />
				sá er of verði glissir,
				<br />
				þótt hann með grömum glami.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Доволен глумливый,
				<br />
				коль, гостя обидев,
				<br />
				удрать ухитрился;
				<br />
				насмешник такой
				<br />
				не знает, что нажил
				<br />
				гневных врагов.
			</span>
		),
	},
	{
		id: 32,
		number: "32.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Gumnar margir
				<br />
				erusk gagnhollir,
				<br />
				en at virði vrekask;
				<br />
				aldar róg
				<br />
				þat mun æ vera,
				<br />
				órir gestr við gest.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Люди друзьями
				<br />
				слывут, но порой
				<br />
				на пиру подерутся;
				<br />
				распри всегда
				<br />
				готовы возникнуть:
				<br />
				гость ссорится с гостем.
			</span>
		),
	},
	{
		id: 33,
		number: "33.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Árliga verðar
				<br />
				skyli maðr oft fáa,
				<br />
				nema til kynnis komi:
				<br />
				sitr ok snópir,
				<br />
				lætr sem solginn sé
				<br />
				ok kann fregna at fáu.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Рано поешь,
				<br />
				а в гости сбираясь,
				<br />
				есть надо плотно:
				<br />
				или голодным
				<br />
				будешь в гостях —<br />
				не сможешь беседовать.
			</span>
		),
	},
	{
		id: 34,
		number: "34.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Afhvarf mikit
				<br />
				er til ills vinar,
				<br />
				þótt á brautu búi,
				<br />
				en til góðs vinar
				<br />
				liggja gagnvegir,
				<br />
				þótt hann sé firr farinn.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Путь неблизок
				<br />
				к другу плохому,
				<br />
				хоть двор его рядом;
				<br />
				а к доброму другу
				<br />
				дорога пряма,
				<br />
				хоть далек его двор.
			</span>
		),
	},
	{
		id: 35,
		number: "35.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ganga skal,
				<br />
				skal-a gestr vera
				<br />
				ey í einum stað;
				<br />
				ljúfr verðr leiðr,
				<br />
				ef lengi sitr
				<br />
				annars fletjum á.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Гость не должен
				<br />
				назойливым быть
				<br />
				и сидеть бесконечно;
				<br />
				даже приятель
				<br />
				станет противен,
				<br />
				коль долго гостит он.
			</span>
		),
	},
	{
		id: 36,
		number: "36.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Bú er betra,
				<br />
				þótt lítit sé,
				<br />
				halr er heima hverr;
				<br />
				þótt tvær geitr eigi
				<br />
				ok taugreftan sal,
				<br />
				þat er þó betra en bæn.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Пусть невелик
				<br />
				твой дом, но твой он,
				<br />
				и в нем ты владыка;
				<br />
				пусть крыша из прутьев
				<br />
				и две лишь козы, —<br />
				это лучше подачек.
			</span>
		),
	},
	{
		id: 37,
		number: "37.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Bú er betra,
				<br />
				þótt lítit sé,
				<br />
				halr er heima hverr;
				<br />
				blóðugt er hjarta,
				<br />
				þeim er biðja skal
				<br />
				sér í mál hvert matar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Пусть невелик
				<br />
				твой дом, но твой он,
				<br />
				и в нем ты владыка;
				<br />
				кровью исходит
				<br />
				сердце у тех,
				<br />
				кто просит подачек.
			</span>
		),
	},
	{
		id: 38,
		number: "38.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Vápnum sínum
				<br />
				skal-a maðr velli á<br />
				feti ganga framar,
				<br />
				því at óvíst er at vita,
				<br />
				nær verðr á vegum úti
				<br />
				geirs of þörf guma.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Муж не должен
				<br />
				хотя бы на миг
				<br />
				отходить от оружья;
				<br />
				ибо как знать,
				<br />
				когда на пути
				<br />
				копье пригодится.
			</span>
		),
	},
	{
		id: 39,
		number: "39.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Fannk-a ek mildan mann
				<br />
				eða svá matar góðan,
				<br />
				at væri-t þiggja þegit,
				<br />
				eða síns féar
				<br />
				svági [glöggvan],
				<br />
				at leið sé laun, ef þægi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Не знаю радушных
				<br />
				и щедрых, что стали б<br />
				дары отвергать;
				<br />
				ни таких, что, в ответ
				<br />
				на подарок врученный,
				<br />
				подарка б не приняли.
			</span>
		),
	},
	{
		id: 40,
		number: "40.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Féar síns,
				<br />
				er fengit hefr,
				<br />
				skyli-t maðr þörf þola;
				<br />
				oft sparir leiðum,
				<br />
				þats hefr ljúfum hugat;
				<br />
				margt gengr verr en varir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Добра не жалей,
				<br />
				что нажито было,
				<br />
				не скорби о потере;
				<br />
				что другу обещано,
				<br />
				недруг возьмет —<br />
				выйдет хуже, чем думалось.
			</span>
		),
	},
	{
		id: 41,
		number: "41.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Vápnum ok váðum
				<br />
				skulu vinir gleðjask;
				<br />
				þat er á sjalfum sýnst;
				<br />
				viðrgefendr ok endrgefendr
				<br />
				erusk lengst vinir,
				<br />
				ef þat bíðr at verða vel.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Оружье друзьям
				<br />
				и одежду дари —<br />
				то тешит их взоры;
				<br />
				друзей одаряя,
				<br />
				ты дружбу крепишь,
				<br />
				коль судьба благосклонна.
			</span>
		),
	},
	{
		id: 42,
		number: "42.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Vin sínum
				<br />
				skal maðr vinr vera
				<br />
				ok gjalda gjöf við gjöf;
				<br />
				hlátr við hlátri
				<br />
				skyli hölðar taka
				<br />
				en lausung við lygi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Надобно в дружбе
				<br />
				верным быть другу,
				<br />
				одарять за подарки;
				<br />
				смехом на смех
				<br />
				пристойно ответить
				<br />и обманом — на ложь.
			</span>
		),
	},
	{
		id: 43,
		number: "43.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Vin sínum
				<br />
				skal maðr vinr vera,
				<br />
				þeim ok þess vin;
				<br />
				en óvinar síns
				<br />
				skyli engi maðr
				<br />
				vinar vinr vera.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Надобно в дружбе
				<br />
				верным быть другу
				<br />
				и другом друзей его;
				<br />
				с недругом друга
				<br />
				никто не обязан
				<br />
				дружбу поддерживать.
			</span>
		),
	},
	{
		id: 44,
		number: "44.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Veiztu, ef þú vin átt,
				<br />
				þann er þú vel trúir,
				<br />
				ok vill þú af hánum gótt geta,
				<br />
				geði skaltu við þann blanda
				<br />
				ok gjöfum skipta,
				<br />
				fara at finna oft.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Если дружбу ведешь
				<br />
				и в друге уверен
				<br />
				и добра ждешь от друга, —<br />
				открывай ему душу,
				<br />
				дары приноси,
				<br />
				навещай его часто.
			</span>
		),
	},
	{
		id: 45,
		number: "45.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ef þú átt annan,
				<br />
				þanns þú illa trúir,
				<br />
				vildu af hánum þó gótt geta,
				<br />
				fagrt skaltu við þann mæla
				<br />
				en flátt hyggja
				<br />
				ok gjalda lausung við lygi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Но если другому
				<br />
				поверил оплошно,
				<br />
				добра ожидая,
				<br />
				сладкою речью
				<br />
				скрой злые мысли
				<br />и лги, если лжет он.
			</span>
		),
	},
	{
		id: 46,
		number: "46.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Það er enn of þann
				<br />
				er þú illa trúir
				<br />
				ok þér er grunr at hans geði,
				<br />
				hlæja skaltu við þeim
				<br />
				ok um hug mæla;
				<br />
				glík skulu gjöld gjöfum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Так же и с теми,
				<br />
				в ком усомнишься,
				<br />
				в ком видишь коварство, —<br />
				улыбайся в ответ,
				<br />
				скрывай свои мысли, —<br />
				тем же отплачивай.
			</span>
		),
	},
	{
		id: 47,
		number: "47.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ungr var ek forðum,
				<br />
				fór ek einn saman,
				<br />
				þá varð ek villr vega;
				<br />
				auðigr þóttumk,
				<br />
				er ek annan fann,
				<br />
				maðr er manns gaman.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Молод я был,
				<br />
				странствовал много
				<br />
				и сбился с пути;
				<br />
				счел себя богачом,
				<br />
				спутника встретив, —<br />
				друг — радость друга.
			</span>
		),
	},
	{
		id: 48,
		number: "48.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Mildir, fræknir
				<br />
				menn bazt lifa,
				<br />
				sjaldan sút ala;
				<br />
				en ósnjallr maðr
				<br />
				uggir hotvetna,
				<br />
				sýtir æ glöggr við gjöfum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Щедрые, смелые
				<br />
				счастливы в жизни,
				<br />
				заботы не знают;
				<br />
				а трус, тот всегда
				<br />
				спасаться готов,
				<br />
				как скупец — от подарка.
			</span>
		),
	},
	{
		id: 49,
		number: "49.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Váðir mínar
				<br />
				gaf ek velli at
				<br />
				tveim trémönnum;
				<br />
				rekkar þat þóttusk,
				<br />
				er þeir rift höfðu;
				<br />
				neiss er nökkviðr halr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				В поле я отдал
				<br />
				одежду мою
				<br />
				двум мужам деревянным;
				<br />
				от этого стали
				<br />
				с людьми они сходны:
				<br />
				жалок нагой.
			</span>
		),
	},
	{
		id: 50,
		number: "50.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Hrörnar þöll,
				<br />
				sú er stendr þorpi á,
				<br />
				hlýr-at henni börkr né barr;
				<br />
				svá er maðr,
				<br />
				sá er manngi ann.
				<br />
				Hvat skal hann lengi lifa?
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Сосна, у дома
				<br />
				возросшая, сохнет,
				<br />
				корой не укрыта;
				<br />
				и человек,
				<br />
				что людям не люб, —<br />
				зачем ему жить!
			</span>
		),
	},
	{
		id: 51,
		number: "51.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Eldi heitari
				<br />
				brennr með illum vinum
				<br />
				friðr fimm daga,
				<br />
				en þá sloknar,
				<br />
				er inn sétti kemr,
				<br />
				ok versnar allr vinskapr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Жарко приязнь
				<br />
				пылает пять дней
				<br />
				меж дурными друзьями;
				<br />
				а пятый прошел —<br />
				погаснет огонь,
				<br />и дружба вся врозь.
			</span>
		),
	},
	{
		id: 52,
		number: "52.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Mikit eitt
				<br />
				skal-a manni gefa;
				<br />
				oft kaupir sér í litlu lof,
				<br />
				með halfum hleif
				<br />
				ok með höllu keri
				<br />
				fekk ek mér félaga.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Подарок большой
				<br />
				не всюду пригоден,
				<br />
				он может быть малым;
				<br />
				неполный кувшин,
				<br />
				половина краюхи
				<br />
				мне добыли друга.
			</span>
		),
	},
	{
		id: 53,
		number: "53.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Lítilla sanda
				<br />
				lítilla sæva
				<br />
				lítil eru geð guma;
				<br />
				því allir menn
				<br />
				urðu-t jafnspakir;
				<br />
				half er öld hvar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				У малых песчинок,
				<br />
				у малых волн
				<br />
				мудрости мало;
				<br />
				не все мудрецы, —<br />
				глупых и умных
				<br />
				поровну в мире.
			</span>
		),
	},
	{
		id: 54,
		number: "54.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Meðalsnotr
				<br />
				skyli manna hverr;
				<br />
				æva til snotr sé;
				<br />
				þeim er fyrða
				<br />
				fegrst at lifa,
				<br />
				er vel margt vitu.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Следует мужу
				<br />
				в меру быть умным,
				<br />
				не мудрствуя много;
				<br />
				лучше живется
				<br />
				тем людям, чьи знанья
				<br />
				не слишком обширны.
			</span>
		),
	},
	{
		id: 55,
		number: "55.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Meðalsnotr
				<br />
				skyli manna hverr,
				<br />
				æva til snotr sé;
				<br />
				því at snotrs manns hjarta
				<br />
				verðr sjaldan glatt,
				<br />
				ef sá er alsnotr, er á.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Следует мужу
				<br />
				в меру быть умным,
				<br />
				не мудрствуя много;
				<br />
				ибо редка
				<br />
				радость в сердцах,
				<br />
				если разум велик.
			</span>
		),
	},
	{
		id: 56,
		number: "56.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Meðalsnotr
				<br />
				skyli manna hverr,
				<br />
				æva til snotr sé;
				<br />
				örlög sín
				<br />
				viti engi fyrir,
				<br />
				þeim er sorgalausastr sefi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Следует мужу
				<br />
				в меру быть умным,
				<br />
				не мудрствуя много;
				<br />
				тот, кто удел свой
				<br />
				не знает вперед,
				<br />
				всего беззаботней.
			</span>
		),
	},
	{
		id: 57,
		number: "57.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Brandr af brandi
				<br />
				brenn, unz brunninn er,
				<br />
				funi kveikisk af funa;
				<br />
				maðr af manni
				<br />
				verðr at máli kuðr,
				<br />
				en til dælskr af dul.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Головня головне
				<br />
				передать готова
				<br />
				пламя от пламени;
				<br />
				в речах человек
				<br />
				познает человека,
				<br />в безмолвье глупеет.
			</span>
		),
	},
	{
		id: 58,
		number: "58.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ár skal rísa,
				<br />
				sá er annars vill
				<br />
				fé eða fjör hafa;
				<br />
				sjaldan liggjandi ulfr
				<br />
				lær of getr
				<br />
				né sofandi maðr sigr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Рано встает,
				<br />
				кто хочет отнять
				<br />
				добро или жизнь;
				<br />
				не видеть добычи
				<br />
				лежачему волку,
				<br />а победы — проспавшему.
			</span>
		),
	},
	{
		id: 59,
		number: "59.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ár skal rísa,
				<br />
				sá er á yrkjendr fáa,
				<br />
				ok ganga síns verka á vit;
				<br />
				margt of dvelr,
				<br />
				þann er um morgin sefr,
				<br />
				hálfr er auðr und hvötum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Рано встает,
				<br />
				кто без подмоги
				<br />
				к труду приступает
				<br />; утром дремота
				<br />
				работе помеха —<br />
				кто бодр, тот богат.
			</span>
		),
	},
	{
		id: 60,
		number: "60.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þurra skíða
				<br />
				ok þakinna næfra,
				<br />
				þess kann maðr mjöt,
				<br />
				þess viðar,
				<br />
				er vinnask megi
				<br />
				mál ok misseri.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Мера бересты
				<br />
				и балок для кровли
				<br />
				известна хозяину,
				<br />
				и сколько потребно
				<br />
				в полгода поленьев
				<br />
				сжигать в очаге.
			</span>
		),
	},
	{
		id: 61,
		number: "61.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þveginn ok mettr
				<br />
				ríði maðr þingi at,
				<br />
				þótt hann sé-t væddr til vel;
				<br />
				skúa ok bróka
				<br />
				skammisk engi maðr
				<br />
				né hests in heldr,
				<br />
				þótt hann hafi-t góðan
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Сытым и чистым
				<br />
				на тинг собирайся,
				<br />
				хоть и в бедной одежде;
				<br />
				сапог и штанов
				<br />
				стыдиться не надо,
				<br />
				а также коня,
				<br />
				коль он неказист.
			</span>
		),
	},
	{
		id: 62,
		number: "62.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Snapir ok gnapir,
				<br />
				er til sævar kemr,
				<br />
				örn á aldinn mar;
				<br />
				svá er maðr,
				<br />
				er með mörgum kemr
				<br />
				ok á formælendr fáa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Вытянув шею,
				<br />
				орел озирает
				<br />
				древнее море;
				<br />
				так смотрит муж,
				<br />
				в чуждой толпе
				<br />
				защиты не знающий.
			</span>
		),
	},
	{
		id: 63,
		number: "63.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Fregna ok segja
				<br />
				skal fróðra hverr,
				<br />
				sá er vill heitinn horskr;
				<br />
				einn vita
				<br />
				né annarr skal,
				<br />
				þjóð veit, ef þrír ro.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Вопросит и ответит
				<br />
				умный всегда,
				<br />
				коль слыть хочет сведущим;
				<br />
				должен один
				<br />
				знать, а не двое, —<br />у трех все проведают.
			</span>
		),
	},
	{
		id: 64,
		number: "64.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ríki sitt
				<br />
				skyli ráðsnotra
				<br />
				hverr í hófi hafa;
				<br />
				þá hann þat finnr,
				<br />
				er með fræknum kemr
				<br />
				at engi er einna hvatastr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Силу свою
				<br />
				должен мудрец
				<br />
				осторожно показывать;
				<br />
				в том убедится
				<br />
				бившийся часто,
				<br />
				что есть и сильнейшие.
			</span>
		),
	},
	{
		id: 65,
		number: "65.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				*********
				<br />
				orða þeira,
				<br />
				er maðr öðrum segir
				<br />
				oft hann gjöld of getr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Бывает, ты слово
				<br />
				скажешь другому,
				<br />а после поплатишься.
			</span>
		),
	},
	{
		id: 66,
		number: "66.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Mikilsti snemma
				<br />
				kom ek í marga staði,
				<br />
				en til síð í suma;
				<br />
				öl var drukkit,
				<br />
				sumt var ólagat,
				<br />
				sjaldan hittir leiðr í líð.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Случалось, я рано
				<br />
				в гости являлся
				<br />
				иль поздно порою:
				<br />
				там выпили пиво,
				<br />
				а там не варили —<br />
				кто не мил, тот некстати.
			</span>
		),
	},
	{
		id: 67,
		number: "67.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Hér ok hvar
				<br />
				myndi mér heim of boðit,
				<br />
				ef þyrftak at málungi mat,
				<br />
				eða tvau lær hengi
				<br />
				at ins tryggva vinar,
				<br />
				þars ek hafða eitt etit.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Повсюду меня
				<br />
				приглашали бы в гости,
				<br />
				но только без трапез
				<br />
				иль если бы, окорок
				<br />
				съевши у друга,
				<br />я два отдавал бы.
			</span>
		),
	},
	{
		id: 68,
		number: "68.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Eldr er beztr
				<br />
				með ýta sonum
				<br />
				ok sólar sýn,
				<br />
				heilyndi sitt,
				<br />
				ef maðr hafa náir,
				<br />
				án við löst at lifa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Драгоценен огонь
				<br />
				для сынов человека
				<br />
				и солнца сиянье;
				<br />
				если телом ты здрав,
				<br />
				то здоровье, а также
				<br />
				жизнь без порока.
			</span>
		),
	},
	{
		id: 69,
		number: "69.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Er-at maðr alls vesall,
				<br />
				þótt hann sé illa heill;
				<br />
				sumr er af sonum sæll,
				<br />
				sumr af frændum,
				<br />
				sumr af fé ærnu,
				<br />
				sumr af verkum vel.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Хворый судьбой
				<br />
				не совсем обездолен:
				<br />
				этот счастлив сынами,
				<br />
				этот близкой родней,
				<br />
				этот богатством,
				<br />а этот деяньями.
			</span>
		),
	},
	{
		id: 70,
		number: "70.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Betra er lifðum
				<br />
				en sé ólifðum,
				<br />
				ey getr kvikr kú;
				<br />
				eld sá ek upp brenna
				<br />
				auðgum manni fyrir,
				<br />
				en úti var dauðr fyr durum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Лучше живым быть,
				<br />
				нежели мертвым;
				<br />
				живой — наживает;
				<br />
				для богатого пламя,
				<br />
				я видел, пылало,
				<br />
				но ждала его смерть.
			</span>
		),
	},
	{
		id: 71,
		number: "71.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Haltr ríðr hrossi,
				<br />
				hjörð rekr handar vanr,
				<br />
				daufr vegr ok dugir,
				<br />
				blindr er betri
				<br />
				en brenndr séi,
				<br />
				nýtr manngi nás.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Ездить может хромой,
				<br />
				безрукий — пасти,
				<br />
				сражаться — глухой;
				<br />
				даже слепец
				<br />
				до сожженья полезен —<br />
				что толку от трупа!
			</span>
		),
	},
	{
		id: 72,
		number: "72.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Sonr er betri,
				<br />
				þótt sé síð of alinn
				<br />
				eftir genginn guma;
				<br />
				sjaldan bautarsteinar
				<br />
				standa brautu nær,
				<br />
				nema reisi niðr at nið.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Сын — это счастье,
				<br />
				хотя бы на свете
				<br />
				отца не застал он;
				<br />
				не будет и камня
				<br />
				у края дороги,
				<br />
				коль сын не поставит.
			</span>
		),
	},
	{
		id: 73,
		number: "73.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Tveir ro eins herjar,
				<br />
				tunga er höfuðs bani;
				<br />
				er mér í heðin hvern
				<br />
				handar væni.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Двое — смерть одному;
				<br />
				голове враг — язык;
				<br />
				под каждым плащом
				<br />
				рука наготове.
			</span>
		),
	},
	{
		id: 74,
		number: "74.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Nótt verðr feginn
				<br />
				sá er nesti trúir,
				<br />
				skammar ro skips ráar;
				<br />
				hverf er haustgríma;
				<br />
				fjölð of viðrir
				<br />
				á fimm dögum
				<br />
				en meira á mánuði.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Ночь тому не страшна,
				<br />
				кто сделал запасы;
				<br />
				коротки реи;
				<br />
				ненастна ночь осенью;
				<br />
				сменится ветер
				<br />
				не раз за пять дней,
				<br />
				несчетно — за месяц.
			</span>
		),
	},
	{
		id: 75,
		number: "75.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Veit-a hinn,
				<br />
				er vettki veit,
				<br />
				margr verðr af aurum api;
				<br />
				maðr er auðigr,
				<br />
				annar óauðigr,
				<br />
				skyli-t þann vítka váar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Иной не постигнет,
				<br />
				что вреден подчас
				<br />
				достаток рассудку;
				<br />
				один — богатей,
				<br />
				другой же — бедняк
				<br />и в том невиновен.
			</span>
		),
	},
	{
		id: 76,
		number: "76.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Deyr fé,
				<br />
				deyja frændr,
				<br />
				deyr sjalfr it sama,
				<br />
				en orðstírr
				<br />
				deyr aldregi,
				<br />
				hveim er sér góðan getr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Гибнут стада,
				<br />
				родня умирает,
				<br />
				и смертен ты сам;
				<br />
				но смерти не ведает
				<br />
				громкая слава
				<br />
				деяний достойных.
			</span>
		),
	},
	{
		id: 77,
		number: "77.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Deyr fé,
				<br />
				deyja frændr,
				<br />
				deyr sjalfr it sama,
				<br />
				ek veit einn,
				<br />
				at aldrei deyr:
				<br />
				dómr um dauðan hvern.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Гибнут стада,
				<br />
				родня умирает,
				<br />
				и смертен ты сам;
				<br />
				но знаю одно,
				<br />
				что вечно бессмертно:
				<br />
				умершего слава.
			</span>
		),
	},
	{
		id: 78,
		number: "78.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Fullar grindr
				<br />
				sá ek fyr Fitjungs sonum,
				<br />
				nú bera þeir vánar völ;
				<br />
				svá er auðr
				<br />
				sem augabragð,
				<br />
				hann er valtastr vina.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				У Фитьюнга были
				<br />
				сыны богачами
				<br />
				и бедность изведали;
				<br />
				может внезапно
				<br />
				исчезнуть достаток —<br />
				друг он неверный.
			</span>
		),
	},
	{
		id: 79,
		number: "79.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ósnotr maðr,
				<br />
				ef eignask getr
				<br />
				fé eða fljóðs munuð,
				<br />
				metnaðr hánum þróask,
				<br />
				en mannvit aldregi,
				<br />
				fram gengr hann drjúgt í dul.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Если глупцу
				<br />
				достается в удел
				<br />
				любовь иль богатство,
				<br />
				не добудет ума он,
				<br />
				но чванство умножит
				<br />и спесью прославится.
			</span>
		),
	},
	{
		id: 80,
		number: "80.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat er þá reynt,
				<br />
				er þú að rúnum spyrr
				<br />
				inum reginkunnum,
				<br />
				þeim er gerðu ginnregin
				<br />
				ok fáði fimbulþulr,
				<br />
				þá hefir hann bazt, ef hann þegir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Вот что отвечу,
				<br />
				когда вопрошаешь
				<br />
				о рунах божественных,
				<br />
				что создали сильные,
				<br />
				а вырезал Вещий:
				<br />
				благо в молчанье.
			</span>
		),
	},
	{
		id: 81,
		number: "81.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				At kveldi skal dag leyfa,
				<br />
				konu, er brennd er,
				<br />
				mæki, er reyndr er,
				<br />
				mey, er gefin er,
				<br />
				ís, er yfir kemr,
				<br />
				öl, er drukkit er.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				День хвали вечером,
				<br />
				жен — на костре,
				<br />
				меч — после битвы,
				<br />
				дев — после свадьбы,
				<br />
				лед — если выдержит,
				<br />
				пиво — коль выпито.
			</span>
		),
	},
	{
		id: 82,
		number: "82.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Í vindi skal við höggva,
				<br />
				veðri á sjó róa,
				<br />
				myrkri við man spjalla,
				<br />
				mörg eru dags augu;
				<br />
				á skip skal skriðar orka,
				<br />
				en á skjöld til hlífar,
				<br />
				mæki höggs,
				<br />
				en mey til kossa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Лес руби на ветру,
				<br />
				жди погоды для гребли,
				<br />
				с девой беседуй
				<br />
				во тьме — зорок день;
				<br />
				у ладьи — быстрота,
				<br />
				у щита — оборона,
				<br />
				удар — у меча,
				<br />
				поцелуи — у девы.
			</span>
		),
	},
	{
		id: 83,
		number: "83.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Við eld skal öl drekka,
				<br />
				en á ísi skríða,
				<br />
				magran mar kaupa,
				<br />
				en mæki saurgan,
				<br />
				heima hest feita,
				<br />
				en hund á búi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Пиво пей у огня,
				<br />
				по льду скользи,
				<br />
				коня купи тощего,
				<br />
				меч — заржавелый,
				<br />
				корми коня дома,
				<br />а пса — у чужих.
			</span>
		),
	},
	{
		id: 84,
		number: "84.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Meyjar orðum
				<br />
				skyli manngi trúa
				<br />
				né því, er kveðr kona,
				<br />
				því at á hverfanda hvéli
				<br />
				váru þeim hjörtu sköpuð,
				<br />
				brigð í brjóst of lagið.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Не доверяй
				<br />
				ни девы речам,
				<br />
				ни жены разговорам —<br />
				на колесе
				<br />
				их слеплено сердце,
				<br />
				коварство в груди их.
			</span>
		),
	},
	{
		id: 85,
		number: "85.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Brestanda boga,
				<br />
				brennanda loga,
				<br />
				gínanda ulfi,
				<br />
				galandi kráku,
				<br />
				rýtanda svíni,
				<br />
				rótlausum viði,
				<br />
				vaxanda vági,
				<br />
				vellanda katli,
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Непрочному луку,
				<br />
				жаркому пламени,
				<br />
				голодному волку,
				<br />
				горластой вороне,
				<br />
				визжащей свинье,
				<br />
				стволу без корней,
				<br />
				встающему валу,
				<br />
				котлу, что кипит,
			</span>
		),
	},
	{
		id: 86,
		number: "86.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Fljúganda fleini,
				<br />
				fallandi báru,
				<br />
				ísi einnættum,
				<br />
				ormi hringlegnum,
				<br />
				brúðar beðmálum
				<br />
				eða brotnu sverði,
				<br />
				bjarnar leiki
				<br />
				eða barni konungs.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Летящей стреле,
				<br />
				отходящему валу,
				<br />
				тонкому льду,
				<br />
				змее, что свилась,
				<br />
				жены объясненьям,
				<br />
				с изъяном мечу,
				<br />
				медведя проделкам,
				<br />и конунга сыну,
			</span>
		),
	},
	{
		id: 87,
		number: "87.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Sjúkum kalfi,
				<br />
				sjalfráða þræli,
				<br />
				völu vilmæli,
				<br />
				val nýfelldum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Скотине больной,
				<br />
				рабу своевольному,
				<br />
				лести колдуньи,
				<br />
				врагу, что сражен,
			</span>
		),
	},
	{
		id: 88,
		number: "88.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Akri ársánum
				<br />
				trúi engi maðr
				<br />
				né til snemma syni,
				<br />
				— veðr ræðr akri.
				<br />
				en vit syni;
				<br />
				hætt er þeira hvárt.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Всходам ранним
				<br />
				не должно нам верить,
				<br />
				ни сыну до срока:
				<br />
				погоде для сева
				<br />
				и сына уму
				<br />
				доверять не дерзай.
			</span>
		),
	},
	{
		id: 89,
		number: "89.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Bróðurbana sínum
				<br />
				þótt á brautu mæti,
				<br />
				húsi hálfbrunnu,
				<br />
				hesti alskjótum,
				<br />
				— þá er jór ónýtr,
				<br />
				ef einn fótr brotnar —,
				<br />
				verði-t maðr svá tryggr
				<br />
				at þessu trúi öllu.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Брата убийце,
				<br />
				коль встречен он будет,
				<br />
				горящему дому,
				<br />
				коню слишком резвому, —<br />
				конь захромает —<br />
				куда он годится, —<br />
				всему, что назвал я,
				<br />
				верить не надо!
			</span>
		),
	},
	{
		id: 90,
		number: "90.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Svá er friðr kvenna,
				<br />
				þeira er flátt hyggja,
				<br />
				sem aki jó óbryddum
				<br />
				á ísi hálum,
				<br />
				teitum, tvévetrum
				<br />
				ok sé tamr illa,
				<br />
				eða í byr óðum
				<br />
				beiti stjórnlausu,
				<br />
				eða skyli haltr henda
				<br />
				hrein í þáfjalli.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Женщин любить,
				<br />
				в обманах искусных, —<br />
				что по льду скакать
				<br />
				на коне без подков,
				<br />
				норовистом, двухлетнем
				<br />
				коне непокорном,
				<br />
				иль в бурю корабль
				<br />
				без кормила вести,
				<br />
				иль хромцу за оленем
				<br />в распутицу гнаться.
			</span>
		),
	},
	{
		id: 91,
		number: "91.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Bert ek nú mæli,
				<br />
				því at ek bæði veit,
				<br />
				brigðr er karla hugr konum;
				<br />
				þá vér fegrst mælum,
				<br />
				er vér flást hyggjum:
				<br />
				þat tælir horska hugi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Откровенно скажу
				<br />
				о мужах и о женах:
				<br />
				мужи тоже лживы;
				<br />
				красно говоря,
				<br />
				но задумав коварство, —<br />
				улестим даже умных.
			</span>
		),
	},
	{
		id: 92,
		number: "92.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Fagrt skal mæla
				<br />
				ok fé bjóða,
				<br />
				sá er vill fljóðs ást fá,
				<br />
				líki leyfa
				<br />
				ins ljósa mans,
				<br />
				sá fær, er fríar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Красно говори
				<br />
				и подарки готовь,
				<br />
				чтобы жен соблазнять;
				<br />
				дев красоту
				<br />
				неустанно хваля,
				<br />
				будь уверен в успехе.
			</span>
		),
	},
	{
		id: 93,
		number: "93.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ástar firna
				<br />
				skyli engi maðr
				<br />
				annan aldregi;
				<br />
				oft fá á horskan,
				<br />
				er á heimskan né fá,
				<br />
				lostfagrir litir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Никто за любовь
				<br />
				никогда осуждать
				<br />
				другого не должен;
				<br />
				часто мудрец
				<br />
				опутан любовью,
				<br />
				глупцу непонятной.
			</span>
		),
	},
	{
		id: 94,
		number: "94.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Eyvitar firna
				<br />
				er maðr annan skal,
				<br />
				þess er um margan gengr guma;
				<br />
				heimska ór horskum
				<br />
				gerir hölða sonu
				<br />
				sá inn máttki munr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Мужей не суди
				<br />
				за то, что может
				<br />
				с каждым свершиться;
				<br />
				нередко бывает
				<br />
				мудрец безрассудным
				<br />
				от сильной страсти.
			</span>
		),
	},
	{
		id: 95,
		number: "95.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Hugr einn þat veit,
				<br />
				er býr hjarta nær,
				<br />
				einn er hann sér of sefa;
				<br />
				öng er sótt verri
				<br />
				hveim snotrum manni
				<br />
				en sér engu at una.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Твоей лишь душе
				<br />
				ведомо то,
				<br />
				что в сердце твоем;
				<br />
				худшей на свете
				<br />
				хвори не знаю,
				<br />
				чем духа томленье.
			</span>
		),
	},
	{
		id: 96,
		number: "96.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat ek þá reynda,
				<br />
				er ek í reyri sat,
				<br />
				ok vættak míns munar;
				<br />
				hold ok hjarta
				<br />
				var mér in horska mær;
				<br />
				þeygi ek hana at heldr hefik.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Изведал я это:
				<br />
				милую ждал я,
				<br />
				таясь в тростниках;
				<br />
				дороже была мне,
				<br />
				чем тело с душой,
				<br />
				но моею не стала.
			</span>
		),
	},
	{
		id: 97,
		number: "97.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Billings mey
				<br />
				ek fann beðjum á<br />
				sólhvíta sofa;
				<br />
				jarls ynði
				<br />
				þótti mér ekki vera
				<br />
				nema við þat lík at lifa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Солнечноясную
				<br />
				Биллинга дочь
				<br />
				нашел я на ложе;
				<br />
				мне ярла власть
				<br />
				не была так желанна,
				<br />
				как светлая дева.
			</span>
		),
	},
	{
		id: 98,
		number: "98.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				«Auk nær aftni
				<br />
				skaltu, Óðinn, koma,
				<br />
				ef þú vilt þér mæla man;
				<br />
				allt eru ósköp,
				<br />
				nema einir viti
				<br />
				slíkan löst saman.»
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				«Вечером, Один,
				<br />
				прийди, чтоб деву
				<br />
				к согласью склонить:
				<br />
				будет неладно,
				<br />
				если другие
				<br />
				про это проведают».
			</span>
		),
	},
	{
		id: 99,
		number: "99.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Aftr ek hvarf
				<br />
				ok unna þóttumk
				<br />
				vísum vilja frá;
				<br />
				hitt ek hugða,
				<br />
				at ek hafa mynda
				<br />
				geð hennar allt ok gaman.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Ее я оставил —<br />
				казалось, от страсти
				<br />
				мой разум мутился;
				<br />
				таил я надежду,
				<br />
				что будет моей
				<br />
				дева любимая.
			</span>
		),
	},
	{
		id: 100,
		number: "100.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Svá kom ek næst,
				<br />
				at in nýta var
				<br />
				vígdrótt öll of vakin
				<br />
				með brennandum ljósum
				<br />
				ok bornum viði,
				<br />
				svá var mér vílstígr of vitaðr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Вновь я пришел,
				<br />
				увидел, что воины
				<br />
				стали стеной, —<br />
				факелы блещут,
				<br />
				завалы из бревен
				<br />
				мне путь преградили.
			</span>
		),
	},
	{
		id: 101,
		number: "101.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Auk nær morgni,
				<br />
				er ek var enn of kominn,
				<br />
				þá var saldrótt of sofin;
				<br />
				grey eitt ek þá fann
				<br />
				innar góðu konu
				<br />
				bundit beðjum á.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				А перед утром, —<br />
				все почивали, —<br />
				явился я вновь;
				<br />
				лишь сука была
				<br />
				привязана к ложу
				<br />
				девы достойной.
			</span>
		),
	},
	{
		id: 102,
		number: "102.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Mörg er góð mær,
				<br />
				ef görva kannar,
				<br />
				hugbrigð við hali;
				<br />
				þá ek þat reynda,
				<br />
				er it ráðspaka
				<br />
				teygða ek á flærðir fljóð;
				<br />
				háðungar hverrar
				<br />
				leitaði mér it horska man,
				<br />
				ok hafða ek þess vettki vífs.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Девы нередко,
				<br />
				коль их разгадаешь,
				<br />
				коварство таят;
				<br />
				изведал я это,
				<br />
				деву пытаясь
				<br />
				к ласкам склонить;
				<br />
				был тяжко унижен
				<br />
				жестокой и все ж<br />
				не достиг я успеха.
			</span>
		),
	},
	{
		id: 103,
		number: "103.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Heima glaðr gumi
				<br />
				ok við gesti reifr,
				<br />
				sviðr skal um sig vera,
				<br />
				minnigr ok málugr,
				<br />
				ef hann vill margfróðr vera,
				<br />
				oft skal góðs geta;
				<br />
				fimbulfambi heitir,
				<br />
				sá er fátt kann segja,
				<br />
				þat er ósnotrs aðal.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Будь дома весел,
				<br />
				будь с гостем приветлив,
				<br />
				но разум храни;
				<br />
				прослыть хочешь мудрым —<br />
				в речах будь искусен, —<br />
				тебя не забудут;
				<br />
				глупцом из глупцов
				<br />
				прослывет безмолвный —<br />
				то свойственно глупым.
			</span>
		),
	},
	{
		id: 104,
		number: "104.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Inn aldna jötun ek sótta,
				<br />
				nú em ek aftr of kominn:
				<br />
				fátt gat ek þegjandi þar;
				<br />
				mörgum orðum
				<br />
				mælta ek í minn frama
				<br />í Suttungs sölum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				От старого турса
				<br />
				вернулся назад я;
				<br />
				промолчал бы — что пользы!
				<br />
				Но речи я вел
				<br />
				и удачи добился
				<br />в палатах у Суттунга.
			</span>
		),
	},
	{
		id: 105,
		number: "105.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Gunnlöð mér of gaf
				<br />
				gullnum stóli á<br />
				drykk ins dýra mjaðar;
				<br />
				ill iðgjöld
				<br />
				lét ek hana eftir hafa
				<br />
				síns ins heila hugar,
				<br />
				síns ins svára sefa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Гуннлед меня
				<br />
				угостила медом
				<br />
				на троне из золота;
				<br />
				плату недобрую
				<br />
				деве я отдал
				<br />
				за ласку, любовь,
				<br />
				за всю ее скорбь.
			</span>
		),
	},
	{
		id: 106,
		number: "106.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Rata munn
				<br />
				létumk rúms of fá
				<br />
				ok um grjót gnaga;
				<br />
				yfir ok undir
				<br />
				stóðumk jötna vegir,
				<br />
				svá hætta ek höfði til.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Рати клыкам
				<br />
				в камень велел я<br />
				крепко вгрызаться;
				<br />
				етунов стены
				<br />
				меня обступили,
				<br />
				мне гибель грозила.
			</span>
		),
	},
	{
		id: 107,
		number: "107.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Vel keypts litar
				<br />
				hefi ek vel notit,
				<br />
				fás er fróðum vant,
				<br />
				því at Óðrerir
				<br />
				er nú upp kominn
				<br />á alda vés jaðar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Хитростью вдоволь
				<br />
				я насладился,
				<br />
				все умный сумеет;
				<br />
				так ныне Одрерир
				<br />
				в доме священном
				<br />
				людей покровителя.
			</span>
		),
	},
	{
		id: 108,
		number: "108.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ifi er mér á,
				<br />
				at ek væra enn kominn
				<br />
				jötna görðum ór,
				<br />
				ef ek Gunnlaðar né nytak,
				<br />
				innar góðu konu,
				<br />
				þeirar er lögðumk arm yfir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Не удалось бы
				<br />
				выбраться мне
				<br />
				из жилья исполинов,
				<br />
				когда бы не помощь
				<br />
				Гуннлед прекрасной,
				<br />
				меня обнимавшей.
			</span>
		),
	},
	{
		id: 109,
		number: "109.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ins hindra dags
				<br />
				gengu hrímþursar
				<br />
				Háva ráðs at fregna
				<br />
				Háva höllu í;
				<br />
				at Bölverki þeir spurðu,
				<br />
				ef hann væri með böndum kominn
				<br />
				eða hefði hánum Suttungr of sóit.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Назавтра собрались
				<br />
				и двинулись хримтурсы
				<br />
				к палатам Высокого
				<br />
				спросить у Высокого:
				<br />
				Бельверк — спросили —<br />
				вернулся к богам
				<br />
				иль сразил его Суттунг?
			</span>
		),
	},
	{
		id: 110,
		number: "110.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Baugeið Óðinn,
				<br />
				hygg ek, at unnit hafi;
				<br />
				hvat skal hans tryggðum trúa?
				<br />
				Suttung svikinn
				<br />
				hann lét sumbli frá
				<br />
				ok grætta Gunnlöðu.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Клятву Один
				<br />
				дал на кольце;
				<br />
				не коварна ли клятва?
				<br />
				Напиток достал он
				<br />
				обманом у Суттунга
				<br />
				Гуннлед на горе.
			</span>
		),
	},
	{
		id: 111,
		number: "111.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Mál er at þylja
				<br />
				þular stóli á<br />
				Urðarbrunni at,
				<br />
				sá ek ok þagðak,
				<br />
				sá ek ok hugðak,
				<br />
				hlýdda ek á manna mál;
				<br />
				of rúnar heyrða ek dæma,
				<br />
				né of ráðum þögðu
				<br />
				Háva höllu at,
				<br />
				Háva höllu í,
				<br />
				heyrða ek segja svá:
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Пора мне с престола
				<br />
				тула поведать
				<br />
				у источника Урд;
				<br />
				смотрел я в молчанье,
				<br />
				смотрел я в раздумье,
				<br />
				слушал слова я;
				<br />
				говорили о рунах,
				<br />
				давали советы
				<br />
				у дома Высокого,
				<br />
				в доме Высокого
				<br />
				так толковали:
			</span>
		),
	},
	{
		id: 112,
		number: "112.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				nótt þú rís-at
				<br />
				nema á njósn séir
				<br />
				eða þú leitir þér innan út staðar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				ночью вставать
				<br />
				по нужде только надо
				<br />
				иль следя за врагом.
			</span>
		),
	},
	{
		id: 113,
		number: "113.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				fjölkunnigri konu
				<br />
				skal-at-tu í faðmi sofa,
				<br />
				svá at hon lyki þik liðum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				с чародейкой не спи,
				<br />
				пусть она не сжимает
				<br />в объятьях тебя.
			</span>
		),
	},
	{
		id: 114,
		number: "114.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Hon svá gerir,
				<br />
				at þú gáir eigi
				<br />
				þings né þjóðans máls;
				<br />
				mat þú vill-at
				<br />
				né mannskis gaman,
				<br />
				ferr þú sorgafullr at sofa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Заставит она
				<br />
				тебя позабыть
				<br />
				о тинге и сходках;
				<br />
				есть не захочешь,
				<br />
				забудешь друзей,
				<br />
				сон горестным станет.
			</span>
		),
	},
	{
		id: 115,
		number: "115.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				annars konu
				<br />
				teygðu þér aldregi
				<br />
				eyrarúnu at.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				чужую жену
				<br />
				не должен ты брать
				<br />в подруги себе.
			</span>
		),
	},
	{
		id: 116,
		number: "116.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				á fjalli eða firði,
				<br />
				ef þik fara tíðir,
				<br />
				fásktu at virði vel.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				в горах ли ты едешь
				<br />
				или по фьордам —<br />
				еды бери вдоволь.
			</span>
		),
	},
	{
		id: 117,
		number: "117.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				illan mann
				<br />
				láttu aldregi
				<br />
				óhöpp at þér vita,
				<br />
				því at af illum manni
				<br />
				fær þú aldregi
				<br />
				gjöld ins góða hugar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				с дурным человеком
				<br />
				несчастьем своим
				<br />
				делиться не должно;
				<br />
				ведь люди дурные
				<br />
				тебе не отплатят
				<br />
				добром за доверье.
			</span>
		),
	},
	{
		id: 118,
		number: "118.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ofarla bíta
				<br />
				ek sá einum hal
				<br />
				orð illrar konu;
				<br />
				fláráð tunga
				<br />
				varð hánum at fjörlagi
				<br />
				ok þeygi of sanna sök.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Я видел однажды,
				<br />
				как муж был погублен
				<br />
				злой женщины словом;
				<br />
				коварный язык
				<br />
				уязвил клеветой,
				<br />
				обвиняя облыжно.
			</span>
		),
	},
	{
		id: 119,
		number: "119.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				veistu, ef þú vin átt,
				<br />
				þann er þú vel trúir,
				<br />
				far þú at finna oft,
				<br />
				því at hrísi vex
				<br />
				ok hávu grasi
				<br />
				vegr, er vættki treðr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				есть друг у тебя,
				<br />
				кому доверяешь, —<br />
				навещай его часто;
				<br />
				высокой травой
				<br />
				и кустами покрыты
				<br />
				неторные тропы.
			</span>
		),
	},
	{
		id: 120,
		number: "120.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				góðan mann
				<br />
				teygðu þér at gamanrúnum
				<br />
				ok nem líknargaldr, meðan þú lifir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				с мужем достойным
				<br />
				мирно беседуй,
				<br />
				добивайся доверья.
			</span>
		),
	},
	{
		id: 121,
		number: "121.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				vin þínum
				<br />
				ver þú aldregi
				<br />
				fyrri at flaumslitum;
				<br />
				sorg etr hjarta,
				<br />
				ef þú segja né náir
				<br />
				einhverjum allan hug.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				дружбу блюди
				<br />
				и первым ее
				<br />
				порвать не старайся;
				<br />
				скорбь твое сердце
				<br />
				сожжет, коль не сможешь
				<br />
				другу довериться.
			</span>
		),
	},
	{
		id: 122,
		number: "122.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				orðum skipta
				<br />
				þú skalt aldregi
				<br />
				við ósvinna apa,
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				глупцу не перечь,
				<br />
				с мужем неумным
				<br />в спор не вступай,
			</span>
		),
	},
	{
		id: 123,
		number: "123.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Því at af illum manni
				<br />
				mundu aldregi
				<br />
				góðs laun of geta,
				<br />
				en góðr maðr
				<br />
				mun þik gerva mega
				<br />
				líknfastan at lofi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Ибо дурной
				<br />
				тебе не отплатит
				<br />
				благом за благо,
				<br />
				а добрый ответит
				<br />
				на дружбу всегда
				<br />
				похвалой и приязнью.
			</span>
		),
	},
	{
		id: 124,
		number: "124.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Sifjum er þá blandat,
				<br />
				hver er segja ræðr
				<br />
				einum allan hug;
				<br />
				allt er betra
				<br />
				en sé brigðum at vera;
				<br />
				er-a sá vinr öðrum, er vilt eitt segir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Хорошему другу
				<br />
				что только хочешь
				<br />
				правдиво поведай;
				<br />
				всегда откровенность
				<br />
				лучше обмана;
				<br />
				не только приятное
				<br />
				другу рассказывай.
			</span>
		),
	},
	{
		id: 125,
		number: "125.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk, þér Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				þrimr orðum senna
				<br />
				skal-at-tu þér við verra mann
				<br />
				oft inn betri bilar,
				<br />
				þá er inn verri vegr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				с тем, кто хуже тебя,
				<br />
				спорить не надо;
				<br />
				нападет негодяй,
				<br />а достойный уступит.
			</span>
		),
	},
	{
		id: 126,
		number: "126.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				skósmiðr þú verir
				<br />
				né skeftismiðr,
				<br />
				nema þú sjalfum þér séir:
				<br />
				skór er skapaðr illa
				<br />
				eða skaft sé rangt,
				<br />
				þá er þér böls beðit.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				обтесывай древки
				<br />
				и обувь готовь
				<br />
				лишь себе самому;
				<br />
				если обувь плоха
				<br />
				или погнуто древко —<br />
				проклятья получишь.
			</span>
		),
	},
	{
		id: 127,
		number: "127.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				hvars þú böl kannt,
				<br />
				kveð þú þér bölvi at
				<br />
				ok gef-at þínum fjándum frið.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				злые поступки
				<br />
				злыми зови,
				<br />
				мсти за злое немедля.
			</span>
		),
	},
	{
		id: 128,
		number: "128.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				illu feginn
				<br />
				ver þú aldregi,
				<br />
				en lát þér at góðu getit.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				дурным никогда
				<br />
				доволен не будь,
				<br />
				дорожи только добрым.
			</span>
		),
	},
	{
		id: 129,
		number: "129.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				upp líta
				<br />
				skal-at-tu í orrustu,
				<br />
				— gjalti glíkir
				<br />
				verða gumna synir, —<br />
				síðr þitt of heilli halir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь;
				<br />
				вверх не смотри,
				<br />
				вступая в сраженье, —<br />
				не сглазил бы враг, —<br />
				воины часто
				<br />
				разум теряют.
			</span>
		),
	},
	{
		id: 130,
		number: "130.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				ef þú vilt þér góða konu
				<br />
				kveðja at gamanrúnum
				<br />
				ok fá fögnuð af,
				<br />
				fögru skaltu heita
				<br />
				ok láta fast vera;
				<br />
				leiðisk manngi gótt, ef getr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				если встречи с красавицей
				<br />
				ищешь и ею
				<br />
				насладиться намерен —<br />
				обещанья давай
				<br />
				и крепко держи их!
				<br />
				Добро не прискучит.
			</span>
		),
	},
	{
		id: 131,
		number: "131.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				varan bið ek þik vera
				<br />
				ok eigi ofvaran;
				<br />
				ver þú við öl varastr
				<br />
				ok við annars konu
				<br />
				ok við þat it þriðja,
				<br />
				at þjófar né leiki.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				будь осторожен,
				<br />
				но страха чуждайся,
				<br />
				пиву не верь
				<br />
				и хитрому вору,
				<br />
				не доверяй
				<br />и жене другого.
			</span>
		),
	},
	{
		id: 132,
		number: "132.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				at háði né hlátri
				<br />
				hafðu aldregi
				<br />
				gest né ganganda.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				потешаться не вздумай
				<br />
				над путником дальним,
				<br />
				глумиться над гостем.
			</span>
		),
	},
	{
		id: 133,
		number: "133.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Oft vitu ógörla,
				<br />
				þeir er sitja inni fyrir,
				<br />
				hvers þeir ro kyns, er koma;
				<br />
				er-at maðr svá góðr
				<br />
				at galli né fylgi,
				<br />
				né svá illr, at einugi dugi.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Не ведают часто
				<br />
				сидящие дома,
				<br />
				кто путник пришедший;
				<br />
				изъян и у доброго
				<br />
				сыщешь, а злой
				<br />
				не во всем нехорош.
			</span>
		),
	},
	{
		id: 134,
		number: "134.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				at hárum þul
				<br />
				hlæ þú aldregi,
				<br />
				oft er gótt, þat er gamlir kveða;
				<br />
				oft ór skörpum belg
				<br />
				skilin orð koma
				<br />
				þeim er hangir með hám
				<br />
				ok skollir með skrám
				<br />
				ok váfir með vílmögum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				над седым стариком
				<br />
				никогда не смейся;
				<br />
				цени слово старца;
				<br />
				цедится мудрость
				<br />
				из старого меха,
				<br />
				что висит возле шкур,
				<br />
				качаясь средь кож,
				<br />с сычугами в соседстве.
			</span>
		),
	},
	{
		id: 135,
		number: "135.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				gest þú né geyja
				<br />
				né á grind hrekir;
				<br />
				get þú váluðum vel.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				над гостями не смейся,
				<br />
				в дверь не гони их,
				<br />к несчастным будь щедр.
			</span>
		),
	},
	{
		id: 136,
		number: "136.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Rammt er þat tré,
				<br />
				er ríða skal
				<br />
				öllum at upploki;
				<br />
				baug þú gef,
				<br />
				eða þat biðja mun
				<br />
				þér læs hvers á liðu.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Ворота сломаешь,
				<br />
				коль всех без разбора
				<br />
				впускать будешь в дом;
				<br />
				кольцо подари,
				<br />
				не то пожеланья
				<br />
				плохие получишь.
			</span>
		),
	},
	{
		id: 137,
		number: "137.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ráðumk þér, Loddfáfnir,
				<br />
				en þú ráð nemir, —<br />
				njóta mundu, ef þú nemr,
				<br />
				þér munu góð, ef þú getr —:
				<br />
				hvars þú öl drekkir,
				<br />
				kjós þér jarðar megin,
				<br />
				því at jörð tekr við ölðri,
				<br />
				en eldr við sóttum,
				<br />
				eik við abbindi,
				<br />
				ax við fjölkynngi,
				<br />
				höll við hýrógi,
				<br />
				— heiftum skal mána kveðja, —<br />
				beiti við bitsóttum,
				<br />
				en við bölvi rúnar,
				<br />
				fold skal við flóði taka.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Советы мои,
				<br />
				Лоддфафнир, слушай,
				<br />
				на пользу их примешь,
				<br />
				коль ты их поймешь:
				<br />
				если ты захмелел —<br />
				землей исцелишься,
				<br />
				ведь землей лечат хмель,
				<br />
				а пламенем — хвори,
				<br />
				понос лечат дубом,
				<br />
				колосьями — порчу,
				<br />
				безумье — луной,
				<br />
				бузиною — желтуху,
				<br />
				червями — укусы
				<br />
				и рунами — чирьи,
				<br />
				земля ж выпьет влагу.
			</span>
		),
	},
	{
		id: 138,
		number: "138.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Veit ek, at ek hekk
				<br />
				vindga meiði á<br />
				nætr allar níu,
				<br />
				geiri undaðr
				<br />
				ok gefinn Óðni,
				<br />
				sjalfr sjalfum mér,
				<br />
				á þeim meiði,
				<br />
				er manngi veit
				<br />
				hvers af rótum renn.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знаю, висел я<br />
				в ветвях на ветру
				<br />
				девять долгих ночей,
				<br />
				пронзенный копьем,
				<br />
				посвященный Одину,
				<br />
				в жертву себе же,
				<br />
				на дереве том,
				<br />
				чьи корни сокрыты
				<br />в недрах неведомых.
			</span>
		),
	},
	{
		id: 139,
		number: "139.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Við hleifi mik sældu
				<br />
				né við hornigi;
				<br />
				nýsta ek niðr,
				<br />
				nam ek upp rúnar,
				<br />
				æpandi nam,
				<br />
				fell ek aftr þaðan.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Никто не питал,
				<br />
				никто не поил меня,
				<br />
				взирал я на землю,
				<br />
				поднял я руны,
				<br />
				стеная их поднял —<br />и с древа рухнул.
			</span>
		),
	},
	{
		id: 140,
		number: "140.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Fimbulljóð níu
				<br />
				nam ek af inum frægja syni
				<br />
				Bölþorns, Bestlu föður,
				<br />
				ok ek drykk of gat
				<br />
				ins dýra mjaðar,
				<br />
				ausinn Óðreri.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Девять песен узнал я<br />
				от сына Бельторна,
				<br />
				Бестлы отца,
				<br />
				меду отведал
				<br />
				великолепного,
				<br />
				что в Одрерир налит.
			</span>
		),
	},
	{
		id: 141,
		number: "141.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þá nam ek frævask
				<br />
				ok fróðr vera
				<br />
				ok vaxa ok vel hafask,
				<br />
				orð mér af orði
				<br />
				orðs leitaði,
				<br />
				verk mér af verki
				<br />
				verks leitaði.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Стал созревать я<br />
				и знанья множить,
				<br />
				расти, процветая;
				<br />
				слово от слова
				<br />
				слово рождало,
				<br />
				дело от дела
				<br />
				дело рождало.
			</span>
		),
	},
	{
		id: 142,
		number: "142.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Rúnar munt þú finna
				<br />
				ok ráðna stafi,
				<br />
				mjök stóra stafi,
				<br />
				mjök stinna stafi,
				<br />
				er fáði fimbulþulr
				<br />
				ok gerðu ginnregin
				<br />
				ok reist hroftr rögna.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Руны найдешь
				<br />
				и постигнешь знаки,
				<br />
				сильнейшие знаки,
				<br />
				крепчайшие знаки,
				<br />
				Хрофт их окрасил,
				<br />
				а создали боги
				<br />и Один их вырезал,
			</span>
		),
	},
	{
		id: 143,
		number: "143.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Óðinn með ásum,
				<br />
				en fyr alfum Dáinn,
				<br />
				Dvalinn ok dvergum fyrir,
				<br />
				Ásviðr jötnum fyrir,
				<br />
				ek reist sjalfr sumar.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Один у асов,
				<br />
				а Даин у альвов,
				<br />
				Двалин у карликов,
				<br />
				у етунов Асвид,
				<br />и сам я их резал.
			</span>
		),
	},
	{
		id: 144,
		number: "144.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Veistu, hvé rísta skal?
				<br />
				Veistu, hvé ráða skal?
				<br />
				Veistu, hvé fáa skal?
				<br />
				Veistu, hvé freista skal?
				<br />
				Veistu, hvé biðja skal?
				<br />
				Veistu, hvé blóta skal?
				<br />
				Veistu, hvé senda skal?
				<br />
				Veistu, hvé sóa skal?
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Умеешь ли резать?
				<br />
				Умеешь разгадывать?
				<br />
				Умеешь окрасить?
				<br />
				Умеешь ли спрашивать?
				<br />
				Умеешь молиться
				<br />
				и жертвы готовить?
				<br />
				Умеешь раздать?
				<br />
				Умеешь заклать?
			</span>
		),
	},
	{
		id: 145,
		number: "145.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Betra er óbeðit
				<br />
				en sé ofblótit,
				<br />
				ey sér til gildis gjöf;
				<br />
				betra er ósent
				<br />
				en sé ofsóit.
				<br />
				Svá Þundr of reist
				<br />
				fyr þjóða rök,
				<br />
				þar hann upp of reis,
				<br />
				er hann aftr of kom.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Хоть совсем не молись,
				<br />
				но не жертвуй без меры,
				<br />
				на дар ждут ответа;
				<br />
				совсем не коли,
				<br />
				чем без меры закалывать.
				<br />
				Так вырезал Тунд
				<br />
				до рожденья людей;
				<br />
				вознесся он там,
				<br />
				когда возвратился.
			</span>
		),
	},
	{
		id: 146,
		number: "146.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Ljóð ek þau kann,
				<br />
				er kann-at þjóðans kona
				<br />
				ok mannskis mögr.
				<br />
				Hjalp heitir eitt,
				<br />
				en þat þér hjalpa mun
				<br />
				við sökum ok sorgum
				<br />
				ok sútum görvöllum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Заклинанья я знаю —<br />
				не знает никто их,
				<br />
				даже конунгов жены;
				<br />
				помощь — такое
				<br />
				первому имя —<br />
				помогает в печалях,
				<br />в заботах и горестях.
			</span>
		),
	},
	{
		id: 147,
		number: "147.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek annat,
				<br />
				er þurfu ýta synir,
				<br />
				þeir er vilja læknar lifa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знаю второе, —<br />
				оно врачеванью
				<br />
				пользу приносит.
			</span>
		),
	},
	{
		id: 148,
		number: "148.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Það kann ek þriðja:
				<br />
				ef mér verðr þörf mikil
				<br />
				hafts við mína heiftmögu,
				<br />
				eggjar ek deyfi
				<br />
				minna andskota,
				<br />
				bíta-t þeim vápn né velir.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знаю и третье, —<br />
				оно защитит
				<br />
				в битве с врагами,
				<br />
				клинки их туплю,
				<br />
				их мечи и дубины
				<br />в бою бесполезны.
			</span>
		),
	},
	{
		id: 149,
		number: "149.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it fjórða:
				<br />
				ef mér fyrðar bera
				<br />
				bönd að boglimum,
				<br />
				svá ek gel,
				<br />
				at ek ganga má,
				<br />
				sprettr mér af fótum fjöturr,
				<br />
				en af höndum haft.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Четвертое знаю, —<br />
				коль свяжут мне члены
				<br />
				оковами крепкими,
				<br />
				так я спою,
				<br />
				что мигом спадут
				<br />
				узы с запястий
				<br />и с ног кандалы.
			</span>
		),
	},
	{
		id: 150,
		number: "150.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it fimmta:
				<br />
				ef ek sé af fári skotinn
				<br />
				flein í folki vaða,
				<br />
				fýgr-a hann svá stinnt,
				<br />
				at ek stöðvig-a-k,
				<br />
				ef ek hann sjónum of sék.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				И пятое знаю, —<br />
				коль пустит стрелу
				<br />
				враг мой в сраженье,
				<br />
				взгляну — и стрела
				<br />
				не долетит,
				<br />
				взору покорная.
			</span>
		),
	},
	{
		id: 151,
		number: "151.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it sétta:
				<br />
				ef mik særir þegn
				<br />
				á vrótum hrás viðar,
				<br />
				ok þann hal
				<br />
				er mik heifta kveðr,
				<br />
				þann eta mein heldr en mik.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знаю шестое, —<br />
				коль недруг корнями
				<br />
				вздумал вредить мне, —<br />
				немедля врага,
				<br />
				разбудившего гнев мой,
				<br />
				несчастье постигнет.
			</span>
		),
	},
	{
		id: 152,
		number: "152.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it sjaunda:
				<br />
				ef ek sé hávan loga
				<br />
				sal of sessmögum,
				<br />
				brennr-at svá breitt,
				<br />
				at ek hánum bjargig-a-k;
				<br />
				þann kann ek galdr at gala.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знаю седьмое, —<br />
				коль дом загорится
				<br />
				с людьми на скамьях,
				<br />
				тотчас я пламя
				<br />
				могу погасить,
				<br />
				запев заклинанье.
			</span>
		),
	},
	{
		id: 153,
		number: "153.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it átta,
				<br />
				er öllum er
				<br />
				nytsamligt at nema:
				<br />
				hvars hatr vex
				<br />
				með hildings sonum
				<br />
				þat má ek bæta brátt.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знаю восьмое, —<br />
				это бы всем
				<br />
				помнить полезно:
				<br />
				где ссора начнется
				<br />
				средь воинов смелых,
				<br />
				могу помирить их.
			</span>
		),
	},
	{
		id: 154,
		number: "154.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it níunda:
				<br />
				ef mik nauðr of stendr
				<br />
				at bjarga fari mínu á floti,
				<br />
				vind ek kyrri
				<br />
				vági á<br />
				ok svæfik allan sæ.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знаю девятое, —<br />
				если ладья
				<br />
				борется с бурей,
				<br />
				вихрям улечься
				<br />
				и волнам утихнуть
				<br />
				пошлю повеленье.
			</span>
		),
	},
	{
		id: 155,
		number: "155.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it tíunda:
				<br />
				ef ek sé túnriður
				<br />
				leika lofti á,
				<br />
				ek svá vinnk,
				<br />
				at þær villar fara
				<br />
				sinna heimhama,
				<br />
				sinna heimhuga.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Знаю десятое, —<br />
				если замечу,
				<br />
				что ведьмы взлетели,
				<br />
				сделаю так,
				<br />
				что не вернуть им
				<br />
				душ своих старых,
				<br />
				обличий оставленных.
			</span>
		),
	},
	{
		id: 156,
		number: "156.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it ellifta:
				<br />
				ef ek skal til orrostu
				<br />
				leiða langvini,
				<br />
				und randir ek gel,
				<br />
				en þeir með ríki fara
				<br />
				heilir hildar til,
				<br />
				heilir hildi frá,
				<br />
				koma þeir heilir hvaðan.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Одиннадцатым
				<br />
				друзей оберечь
				<br />
				в битве берусь я,
				<br />
				в щит я пою, —<br />
				побеждают они,
				<br />
				в боях невредимы,
				<br />
				из битв невредимы
				<br />
				прибудут с победой.
				<br />
			</span>
		),
	},
	{
		id: 157,
		number: "157.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it tolfta:
				<br />
				ef ek sé á tré uppi
				<br />
				váfa virgilná,
				<br />
				svá ek ríst
				<br />
				ok í rúnum fák,
				<br />
				at sá gengr gumi
				<br />
				ok mælir við mik.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Двенадцатым я,
				<br />
				увидев на дереве
				<br />
				в петле повисшего,
				<br />
				так руны вырежу,
				<br />
				так их окрашу,
				<br />
				что он оживет
				<br />и беседовать будет.
			</span>
		),
	},
	{
		id: 158,
		number: "158.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it þrettánda:
				<br />
				ef ek skal þegn ungan
				<br />
				verpa vatni á,
				<br />
				mun-at hann falla,
				<br />
				þótt hann í folk komi,
				<br />
				hnígr-a sá halr fyr hjörum.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Тринадцатым я<br />
				водою младенца
				<br />
				могу освятить, —<br />
				не коснутся мечи его,
				<br />
				и невредимым
				<br />в битвах он будет.
			</span>
		),
	},
	{
		id: 159,
		number: "159.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it fjögurtánda:
				<br />
				ef ek skal fyrða liði
				<br />
				telja tíva fyrir,
				<br />
				ása ok alfa
				<br />
				ek kann allra skil;
				<br />
				fár kann ósnotr svá.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Четырнадцатым
				<br />
				число я открою
				<br />
				асов и альвов,
				<br />
				прозванье богов
				<br />
				поведаю людям, —<br />
				то может лишь мудрый.
			</span>
		),
	},
	{
		id: 160,
		number: "160.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it fimmtánda
				<br />
				er gól Þjóðrerir
				<br />
				dvergr fyr Dellings durum:
				<br />
				afl gól hann ásum,
				<br />
				en alfum frama,
				<br />
				hyggju Hroftatý.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Пятнадцатое
				<br />
				Тьодрерир пел
				<br />
				пред дверью Деллинга;
				<br />
				напел силу асам,
				<br />
				и почести — альвам,
				<br />и Одину — дух.
			</span>
		),
	},
	{
		id: 161,
		number: "161.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it sextánda:
				<br />
				ef ek vil ins svinna mans
				<br />
				hafa geð allt ok gaman,
				<br />
				hugi ek hverfi
				<br />
				hvítarmri konu,
				<br />
				ok sný ek hennar öllum sefa.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Шестнадцатым я<br />
				дух шевельну
				<br />
				девы достойной,
				<br />
				коль дева мила,
				<br />
				овладею душой,
				<br />
				покорю ее помыслы.
			</span>
		),
	},
	{
		id: 162,
		number: "162.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it sjautjánda
				<br />
				at mik mun seint firrask
				<br />
				it manunga man.
				<br />
				Ljóða þessa
				<br />
				mun þú, Loddfáfnir,
				<br />
				lengi vanr vera;
				<br />
				þó sé þér góð, ef þú getr,
				<br />
				nýt ef þú nemr,
				<br />
				þörf ef þú þiggr.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Семнадцатым я<br />
				опутать смогу
				<br />
				душу девичью;
				<br />
				те заклятья, Лоддфафнир,
				<br />
				будут тебе
				<br />
				навек неизвестны;
				<br />
				хотя хороши они,
				<br />
				впрок бы принять их,
				<br />
				на пользу усвоить.
			</span>
		),
	},
	{
		id: 163,
		number: "163.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Þat kann ek it átjánda,
				<br />
				er ek æva kennik
				<br />
				mey né manns konu,
				<br />
				— allt er betra,
				<br />
				er einn of kann;
				<br />
				þat fylgir ljóða lokum, —<br />
				nema þeiri einni,
				<br />
				er mik armi verr,
				<br />
				eða mín systir sé.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Восемнадцатое
				<br />
				ни девам, ни женам
				<br />
				сказать не смогу я, —<br />
				один сбережет
				<br />
				сокровеннее тайну, —<br />
				тут песня пресеклась —<br />
				откроюсь, быть может,
				<br />
				только жене
				<br />
				иль сестре расскажу.
			</span>
		),
	},
	{
		id: 164,
		number: "164.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40">
				Nú eru Háva mál
				<br />
				kveðin Háva höllu í,
				<br />
				allþörf ýta sonum,
				<br />
				óþörf jötna sonum;
				<br />
				heill sá, er kvað,
				<br />
				heill sá, er kann,
				<br />
				njóti sá, er nam,
				<br />
				heilir, þeirs hlýddu.
			</span>
		),

		contentRU: (
			<span className="flex w-40">
				Вот речи Высокого
				<br />
				в доме Высокого,
				<br />
				нужные людям,
				<br />
				ненужные етунам.
				<br />
				Благо сказавшему!
				<br />
				Благо узнавшим!
				<br />
				Кто вспомнит — воспользуйся!
				<br />
				Благо внимавшим!
			</span>
		),
	},
];

export { highsspeeches };
