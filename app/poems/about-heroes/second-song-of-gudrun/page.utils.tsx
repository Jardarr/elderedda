const secondsongofgudrun = [
	{
		id: 1,
		class: "m-7",

		contentON: (
			<span>
				<p className="mb-2">
					Þjóðrekr konungr var með Atla ok hafði þar látit flesta alla menn
					sína. Þjóðrekr ok Guðrún kærðu harma sín á milli. Hon sagði hánum ok
					kvað:
				</p>
			</span>
		),

		contentRU: (
			<span>
				<p>
					Конунг Тьодрек был у Атли и потерял там большую часть своих людей.
					Тьодрек и Гудрун жаловались друг другу на свои несчастья. Она сказала
					ему,
				</p>
			</span>
		),
	},
	{
		id: 2,
		number: "1.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Mær var ek meyja,
				<br />
				— móðir mik fæddi, —<br />
				björt í búri,
				<br />
				unna ek vel bræðrum, —<br />
				unz mik Gjúki
				<br />
				gulli reifði,
				<br />
				gulli reifði,
				<br />
				gaf Sigurði.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				«Девой счастливейшей
				<br />
				в женских хоромах
				<br />
				я родилась,
				<br />
				любила я братьев,
				<br />
				покуда мне Гьюки
				<br />
				золота не дал, —<br />
				золото дал он
				<br />и выдал за Сигурда.
			</span>
		),
	},
	{
		id: 3,
		number: "2.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Svá var Sigurðr
				<br />
				of sonum Gjúka
				<br />
				sem væri grænn laukr
				<br />
				ór grasi vaxinn
				<br />
				eða hjörtr hábeinn
				<br />
				of hvössum dýrum
				<br />
				eða gull glóðrautt
				<br />
				af gráu silfri.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Таким был Сигурд
				<br />
				пред Гьюки сынами,
				<br />
				как стебель лука,
				<br />
				из трав встающий,
				<br />
				как легкий олень
				<br />
				меж тварей лесных,
				<br />
				как золота пламя
				<br />
				пред оловом тусклым.
			</span>
		),
	},
	{
		id: 4,
		number: "3.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Unz mér fyrmunðu
				<br />
				mínir bræðr,
				<br />
				at ek ætta ver
				<br />
				öllum fremra;
				<br />
				sofa þeir né máttu-t
				<br />
				né of sakar dæma,
				<br />
				áðr þeir Sigurð
				<br />
				svelta létu.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Зависть братьев
				<br />
				моих обуяла —<br />
				муж мой был лучшим
				<br />
				между героями;
				<br />
				спать не могли,
				<br />
				ни дела обсуждать,
				<br />
				пока они Сигурда
				<br />
				не погубили.
			</span>
		),
	},
	{
		id: 5,
		number: "4.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Grani rann at þingi,
				<br />
				— gnýr var at heyra, —<br />
				en þá Sigurðr
				<br />
				sjalfr eigi kom;
				<br />
				öll váru söðuldýr
				<br />
				sveita stokkin
				<br />
				ok of vanið vási
				<br />
				und vegöndum.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Грани примчался, —<br />
				слышен был топот, —<br />
				Сигурд тогда
				<br />
				сам не приехал;
				<br />
				были все кони
				<br />
				обрызганы кровью,
				<br />
				в пути утомясь,
				<br />
				убийц привезли они.
			</span>
		),
	},
	{
		id: 6,
		number: "5.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Gekk ek grátandi
				<br />
				við Grana ræða,
				<br />
				úrughlýra
				<br />
				jó frá ek spjalla;
				<br />
				hnipnaði Grani þá,
				<br />
				drap í gras höfði,
				<br />
				jór þat vissi,
				<br />
				eigendr né lifðu-t.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				С Грани пошла я<br />
				беседовать, плача,
				<br />
				стала расспрашивать,
				<br />
				слезы роняя;
				<br />
				Грани понурил
				<br />
				голову низко —<br />
				знал о беде он:
				<br />
				не стало хозяина.
			</span>
		),
	},
	{
		id: 7,
		number: "6.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Lengi hvarfaðak,
				<br />
				lengi hugir deildusk,
				<br />
				áðr ek of frægak
				<br />
				folkvörð at gram.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Долго терзалась я,
				<br />
				долго молчала,
				<br />
				все же спросить
				<br />
				решилась у Гуннара.
			</span>
		),
	},
	{
		id: 8,
		number: "7.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Hnipnaði Gunnarr,
				<br />
				sagði mér Högni
				<br />
				frá Sigurðar
				<br />
				sárum dauða:
				<br />
				«Liggr of höggvinn
				<br />
				fyr handan ver
				<br />
				Gothorms bani
				<br />
				of gefinn ulfum.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Голову скорбно
				<br />
				Гуннар склонил;
				<br />
				Хегни сказал мне
				<br />
				о смерти жестокой:
				<br />
				«Лежит изрубленный
				<br />
				там за рекой
				<br />
				убивший Готторма, —<br />
				отдан волкам он.
			</span>
		),
	},
	{
		id: 9,
		number: "8.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Líttu þar Sigurð
				<br />
				á suðrvega;
				<br />
				þá heyrir þú
				<br />
				hrafna gjalla,
				<br />
				örnu gjalla
				<br />
				æzli fegna,
				<br />
				varga þjóta
				<br />
				of veri þínum.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Взгляни на юг —<br />
				вот Сигурд лежит!
				<br />
				Слушай, как воронов
				<br />
				каркает стая,
				<br />
				добычу орлы
				<br />
				с клекотом делят,
				<br />
				волки над мужем
				<br />
				твоим завывают».
			</span>
		),
	},
	{
		id: 10,
		number: "9.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				«Hví þú mér, Högni,
				<br />
				harma slíka
				<br />
				viljalaussi
				<br />
				vill of segja?
				<br />
				Þitt skyli hjarta
				<br />
				hrafnar slíta
				<br />
				við lönd yfir
				<br />
				en þú vitir manna!»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гудрун сказала:
				<br />
				«Хегни, зачем ты
				<br />
				счастья лишенной
				<br />
				о горе подобном
				<br />
				поведать вздумал?
				<br />
				Пусть сердце твое
				<br />
				ворон терзает
				<br />
				в далекой земле,
				<br />
				которой не знаешь ты».
			</span>
		),
	},
	{
		id: 11,
		number: "10.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Svaraði Högni
				<br />
				sinni einu,
				<br />
				trauðr góðs hugar,
				<br />
				af trega stórum:
				<br />
				«Þess áttu, Guðrún,
				<br />
				græti at fleiri,
				<br />
				at hjarta mitt
				<br />
				hrafnar slíti.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Одно лишь Хегни
				<br />
				молвил в ответ, —<br />
				сумрачен был он
				<br />
				от сильного горя:
				<br />
				«Гудрун, тебе бы
				<br />
				сильней горевать
				<br />
				о том, что сердце
				<br />
				склюют мне вороны!»
			</span>
		),
	},
	{
		id: 12,
		number: "11.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Hvarf ek ein þaðan
				<br />
				annspilli frá
				<br />
				á við lesa
				<br />
				varga leifar;
				<br />
				gerðig-a ek hjúfra
				<br />
				né höndum slá
				<br />
				né kveina um
				<br />
				sem konur aðrar,
				<br />
				þá er sat soltin
				<br />
				of Sigurði.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Одна я ушла
				<br />
				после этой беседы
				<br />
				в лес, чтобы взять
				<br />
				волчью добычу:
				<br />
				не голосила,
				<br />
				руки ломая,
				<br />
				не причитала,
				<br />
				как жены другие,
				<br />
				как мертвая, сидя
				<br />
				над телом Сигурда.
			</span>
		),
	},
	{
		id: 13,
		number: "12.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Nótt þótti mér
				<br />
				niðmyrkr vera,
				<br />
				er ek sárla satk
				<br />
				yfir Sigurði;
				<br />
				ulfar þóttumk
				<br />
				öllu betri,
				<br />
				ef þeir léti mik
				<br />
				lífi týna
				<br />
				eða brenndi mik
				<br />
				sem birkinn við.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Ночь мне казалась —<br />
				как в новолунье,
				<br />
				когда над Сигурдом
				<br />
				в горе сидела я;
				<br />
				мнилось, что волки
				<br />
				благо бы сделали,
				<br />
				если б меня
				<br />
				жизни лишили!
				<br />
				Если б сгорела
				<br />
				я, как береза!
			</span>
		),
	},
	{
		id: 14,
		number: "13.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Fór ek af fjalli
				<br />
				fimm dægr talið,
				<br />
				unz ek höll Halfs
				<br />
				háva þekkðak.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Пять дней я спускалась
				<br />
				по горным склонам,
				<br />
				пока не увидела
				<br />
				Хальва палаты.
			</span>
		),
	},
	{
		id: 15,
		number: "14.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Sat ek með Þóru
				<br />
				sjau misseri,
				<br />
				dætr Hákonar
				<br />
				í Danmörku;
				<br />
				hon mér at gamni
				<br />
				gullbókaði
				<br />
				sali suðræna
				<br />
				ok svani danska.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Прожила я у Торы
				<br />
				семь полугодий,
				<br />
				у дочери Хакона
				<br />
				в датской земле.
				<br />
				Шитьем золотым
				<br />
				меня забавляла,
				<br />
				вышивая палаты
				<br />и витязей датских.
			</span>
		),
	},
	{
		id: 16,
		number: "15.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Hafðu vit á skriftum
				<br />
				þat er skatar léku,
				<br />
				ok á hannyrðum
				<br />
				hilmis þegna,
				<br />
				randir rauðar,
				<br />
				rekka Húna,
				<br />
				hjördrótt, hjalmdrótt,
				<br />
				hilmis fylgju.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Вышили с ней мы
				<br />
				конунгов подвиги,
				<br />
				были на тканях
				<br />
				воины князя,
				<br />
				щиты червленые,
				<br />
				гуннов воители,
				<br />
				с мечами и в шлемах
				<br />
				княжья дружина;
			</span>
		),
	},
	{
		id: 17,
		number: "16.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Skip Sigmundar
				<br />
				skriðu frá landi,
				<br />
				gylltar grímur,
				<br />
				grafnir stafnar;
				<br />
				byrðu vit á borða,
				<br />
				þat er þeir börðusk
				<br />
				Sigarr ok Siggeirr
				<br />
				suðr á Fjóni.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				По морю струги
				<br />
				Сигмунда плыли —<br />
				драконьи морды
				<br />
				и штевни резные;
				<br />
				вышили мы,
				<br />
				как бились на юге
				<br />
				Сигар и Сиггейр
				<br />
				на острове Фьоне.
			</span>
		),
	},
	{
		id: 18,
		number: "17.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Þá frá Grimildr,
				<br />
				gotnesk kona,
				<br />
				hvat ek væra
				<br />
				hyggjuð *********;
				<br />
				hon brá borða
				<br />
				ok buri heimti
				<br />
				þrágjarnliga
				<br />
				þess at spyrja,
				<br />
				hverr vildi son
				<br />
				systur bæta
				<br />
				eða ver veginn
				<br />
				vildi gjalda.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Проведала Гримхильд,
				<br />
				готская женщина,
				<br />
				где я живу
				<br />
				Оставила вышивку,
				<br />
				вызвала сына,
				<br />
				чтобы спросить
				<br />
				своенравного воина,
				<br />
				согласен ли он
				<br />
				сестре отплатить
				<br />
				иль выкуп за мертвого
				<br />
				мужа отдаст он.
			</span>
		),
	},
	{
		id: 19,
		number: "18.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Görr lézk Gunnarr
				<br />
				gull at bjóða
				<br />
				sakar at bæta,
				<br />
				ok it sama Högni.
				<br />
				Hon frétti at því,
				<br />
				hverr fara vildi
				<br />
				vigg at söðla
				<br />
				vagn at beita,
				<br />
				hesti ríða,
				<br />
				hauki fleygja,
				<br />
				öðrum at skjóta
				<br />
				af ýboga.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гуннар готов был —<br />
				и Хегни тоже —<br />
				выплатить золото,
				<br />
				выкуп за распрю.
				<br />
				Спросила она,
				<br />
				кто согласится
				<br />
				коня оседлать,
				<br />
				в повозку запрячь,
				<br />
				скакать на коне,
				<br />
				сокола взвить,
				<br />
				луком из тиса
				<br />
				стрелы пуская.
			</span>
		),
	},
	{
		id: 20,
		number: "19.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Valdarr Dönum
				<br />
				með Jarizleifi,
				<br />
				Eymóðr þriði
				<br />
				með Jarizskári
				<br />
				inn gengu þá,
				<br />
				jöfrum líkir,
				<br />
				Langbarðs liðar,
				<br />
				höfðu loða rauða,
				<br />
				stuttar brynjur,
				<br />
				steypða hjalma,
				<br />
				skalmum gyrðir,
				<br />
				höfðu skarar jarpar.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Вальдар датский,
				<br />
				и Ярицлейв с ним,
				<br />
				Эймод третий,
				<br />
				а с ними и Ярицкар
				<br />
				в палату вошли,
				<br />
				подобны князьям,
				<br />
				Лангбарда воины,
				<br />
				в красных плащах,
				<br />
				кольчуги их в золоте,
				<br />
				острые шлемы,
				<br />
				мечи у бедра,
				<br />
				волосы темные.
			</span>
		),
	},
	{
		id: 21,
		number: "20.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Hverr vildi mér
				<br />
				hnossir velja,
				<br />
				hnossir velja
				<br />
				ok hugat mæla,
				<br />
				ef þeir mætti mér
				<br />
				margra súta
				<br />
				tryggðir vinna, —<br />
				né ek trúa gerða.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Каждый пытался
				<br />
				подарок вручить мне,
				<br />
				подарок вручить
				<br />
				и в печали утешить,
				<br />
				как будто могли
				<br />
				горе мое
				<br />
				этим унять,
				<br />
				но им я не верила.
			</span>
		),
	},
	{
		id: 22,
		number: "21.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Færði mér Grímhildr
				<br />
				full at drekka
				<br />
				svalt ok sárligt,
				<br />
				né ek sakar munðak;
				<br />
				þat var of aukit
				<br />
				jarðar magni,
				<br />
				svalköldum sæ
				<br />
				ok sónum dreyra.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гримхильд напиток
				<br />
				мне поднесла
				<br />
				терпкий, студеный,
				<br />
				чтоб горе забыла я:
				<br />
				сдобрен он был
				<br />
				силой земли,
				<br />
				холодной волной
				<br />и кровью вепря.
			</span>
		),
	},
	{
		id: 23,
		number: "22.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Váru í horni
				<br />
				hvers kyns stafir
				<br />
				ristnir ok roðnir,
				<br />
				— ráða ek né máttak, —<br />
				lyngfiskr langr,
				<br />
				lands Haddingja
				<br />
				ax óskorit,
				<br />
				innleið dyra.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Были на роге
				<br />
				багряные руны —<br />
				что они значат,
				<br />
				прочесть не могла я:
				<br />
				вереска рыба,
				<br />
				Хаддинги края
				<br />
				несрезанный колос,
				<br />
				звериная пасть.
			</span>
		),
	},
	{
		id: 24,
		number: "23.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Váru þeim bjóri
				<br />
				böl mörg saman,
				<br />
				urt alls viðar
				<br />
				ok akarn brunnin,
				<br />
				umdögg arins,
				<br />
				iðrar blótnar,
				<br />
				svíns lifr soðin,
				<br />
				því at hon sakar deyfði.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Были в той браге
				<br />
				многие беды,
				<br />
				листья и желудя
				<br />
				жженого пепел,
				<br />
				роса очага
				<br />
				и жертв требуха,
				<br />
				печень свиная,
				<br />
				свары гасящая.
			</span>
		),
	},
	{
		id: 25,
		number: "24.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				En þá gleymðu,
				<br />
				er getit höfðu,
				<br />
				öll jöfurs
				<br />
				jórbjúg í sal;
				<br />
				kómu konungar
				<br />
				fyr kné þrennir,
				<br />
				áðr hon sjalfa mik
				<br />
				sótti at máli.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Забыли тогда,
				<br />
				что совершили
				<br />
				[непонятное место]
				<br />
				Трое князей
				<br />
				ко мне подошли,
				<br />
				прежде чем мне она
				<br />
				молвила слово.
			</span>
		),
	},
	{
		id: 26,
		number: "25.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Grímhildr kvað:
				<br />
				«Gef ek þér, Guðrún,
				<br />
				gull at þiggja,
				<br />
				fjölð alls féar,
				<br />
				at þinn föður dauðan,
				<br />
				hringa rauða,
				<br />
				Hlöðvés sali,
				<br />
				ársal allan
				<br />
				at jöfur fallinn.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				«Дам тебе, Гудрун,
				<br />
				золота груду,
				<br />
				все, что отец
				<br />
				в наследье оставил,
				<br />
				кольца червленые,
				<br />
				Хледвера земли,
				<br />
				ковер драгоценный
				<br />
				за конунга мертвого.
			</span>
		),
	},
	{
		id: 27,
		number: "26.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Húnskar meyjar,
				<br />
				þær er hlaða spjöldum
				<br />
				ok gera gull fagrt,
				<br />
				svá at þér gaman þykki;
				<br />
				ein skaltu ráða
				<br />
				auði Buðla,
				<br />
				gulli göfguð
				<br />
				ok gefin Atla.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Девушек гуннских,
				<br />
				ткущих искусно,
				<br />
				золотом шьющих
				<br />
				тебе на забаву, —<br />
				Будли сокровища
				<br />
				будут твоими,
				<br />
				вся в золоте выйдешь
				<br />
				замуж за Атли!»
			</span>
		),
	},
	{
		id: 28,
		number: "27.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Guðrún kvað:
				<br />
				«Vilk eigi ek
				<br />
				með veri ganga
				<br />
				né Brynhildar
				<br />
				bróður eiga;
				<br />
				samir eigi mér
				<br />
				við son Buðla
				<br />
				ætt at auka
				<br />
				né una lífi.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гудрун сказала:
				<br />
				«Нет, не бывать
				<br />
				браку такому,
				<br />
				никогда я за брата
				<br />
				Брюнхильд не выйду!
				<br />
				Мне не пристало
				<br />
				с отпрыском Будли
				<br />
				род умножать
				<br />и радостно жить!»
			</span>
		),
	},
	{
		id: 29,
		number: "28.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Grímhildr kvað:
				<br />
				«Hirð-a-ðu hölðum
				<br />
				heiftir gjalda,
				<br />
				því at vér höfum
				<br />
				valdit fyrri;
				<br />
				svá skaltu láta,
				<br />
				sem þeir lifi báðir
				<br />
				Sigurðr ok Sigmundr,
				<br />
				ef þú sonu fæðir.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гримхильд сказала:
				<br />
				«Враждебной не будь
				<br />
				к героям воителям,
				<br />
				хоть и повинны
				<br />
				мы пред тобою!
				<br />
				Снова все будет,
				<br />
				как если б жили
				<br />
				Сигурд и Сигмунд, —<br />
				роди сыновей лишь!»
			</span>
		),
	},
	{
		id: 30,
		number: "29.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Guðrún kvað:
				<br />
				«Mák-a ek, Grímhildr,
				<br />
				glaumi bella
				<br />
				né vígrisins
				<br />
				vánir telja,
				<br />
				siz Sigurðar
				<br />
				sárla drukku
				<br />
				hrægífr, huginn
				<br />
				hjartblóð saman.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гудрун сказала:
				<br />
				«Гримхильд, постылы мне
				<br />
				шумные пиршества,
				<br />
				как уступлю я<br />
				желаниям Атли,
				<br />
				если трупов чудовище
				<br />
				с Хугином вместе
				<br />
				Сигурда кровь
				<br />
				пили из сердца!»
			</span>
		),
	},
	{
		id: 31,
		number: "30.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Grímhildr kvað:
				<br />
				«Þann hefi ek allra
				<br />
				ættgöfgastan
				<br />
				fylki fundit
				<br />
				ok framast nökkvi;
				<br />
				hann skaltu eiga,
				<br />
				unz þik aldr viðr,
				<br />
				verlaus vera,
				<br />
				nema þú vilir þenna.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гримхильд сказала:
				<br />
				«Великого конунга
				<br />
				я тебе выбрала,
				<br />
				первым из всех
				<br />
				он признан повсюду;
				<br />
				с ним проживешь ты
				<br />
				до самой смерти,
				<br />
				а не захочешь —<br />
				не быть тебе замужем!»
			</span>
		),
	},
	{
		id: 32,
		number: "31.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Guðrún kvað:
				<br />
				«Hirð-a-ðu bjóða
				<br />
				bölvafullar
				<br />
				þrágjarnliga
				<br />
				þær kindir mér;
				<br />
				hann mun Gunnar
				<br />
				grandi beita
				<br />
				ok ór Högna
				<br />
				hjarta slíta;
				<br />
				munk-at ek létta,
				<br />
				áðr lífshvatan
				<br />
				eggleiks hvötuð
				<br />
				aldri næmik.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гудрун сказала:
				<br />
				«Нет, не стремись,
				<br />
				к сварам привычная,
				<br />
				злое родство
				<br />
				мне навязать!
				<br />
				Гуннару он
				<br />
				зло причинит,
				<br />
				сердце у Хегни
				<br />
				вырвет из ребер.
				<br />
				Не буду спокойной,
				<br />
				пока не убью
				<br />
				того, кто забавы
				<br />
				мечей затевает».
			</span>
		),
	},
	{
		id: 33,
		number: "32.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Grátandi Grímhildr
				<br />
				greip við orði,
				<br />
				er burum sínum
				<br />
				bölva vætti
				<br />
				ok mögum sínum
				<br />
				meina stórra.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Горько рыдая,
				<br />
				молвила Гримхильд,
				<br />
				беды сынов
				<br />
				и родичей видя,
				<br />
				злые напасти
				<br />
				для них ожидая:
			</span>
		),
	},
	{
		id: 34,
		number: "33.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Grímhildr kvað:
				<br />
				«Lönd gef ek enn þér,
				<br />
				lýða sinni,
				<br />
				Vinbjörg, Valbjörg,
				<br />
				ef þú vill þiggja;
				<br />
				eigðu um aldr þat
				<br />
				ok uni, dóttir.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				«Еще я дам земли
				<br />
				и с ними дружину,
				<br />
				Винбьерг и Вальбьерг,
				<br />
				коль взять ты согласна, —<br />
				до смерти владей
				<br />и счастлива будь!»
			</span>
		),
	},
	{
		id: 35,
		number: "34.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Guðrún kvað:
				<br />
				«Þann mun ek kjósa
				<br />
				af konungum
				<br />
				ok þó af niðjum
				<br />
				nauðig hafa;
				<br />
				verðr eigi mér
				<br />
				verr at ynði
				<br />
				né böl bræðra
				<br />
				at bura skjóli.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гудрун сказала:
				<br />
				«Выбор я сделаю,
				<br />
				конунга выберу,
				<br />
				но так поступить
				<br />
				родня принуждает:
				<br />
				не суждено мне
				<br />
				счастливой быть с мужем,
				<br />
				братьев беда
				<br />
				не спасет сыновей!»
			</span>
		),
	},
	{
		id: 36,
		number: "35.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Senn var á hesti
				<br />
				hverr drengr litinn,
				<br />
				en víf valnesk
				<br />
				hafið í vagna;
				<br />
				vér sjau daga
				<br />
				svalt land riðum,
				<br />
				en aðra sjau
				<br />
				unnir kníðum,
				<br />
				en ina þriðju sjau
				<br />
				þurrt land stigum.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Воины все
				<br />
				на коней вскочили,
				<br />
				вальские жены
				<br />
				сели в повозки:
				<br />
				семь дней мы ехали
				<br />
				по землям студеным,
				<br />
				семь дней веслами
				<br />
				волны месили
				<br />
				и семь еще дней
				<br />
				посуху шли.
			</span>
		),
	},
	{
		id: 37,
		number: "36.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Þar hliðverðir
				<br />
				hárar borgar
				<br />
				grind upp luku,
				<br />
				áðr í garð riðum.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Тогда вратари
				<br />
				высокого града
				<br />
				открыли ворота,
				<br />
				чтоб въехать во двор нам.
			</span>
		),
	},
	{
		id: 38,
		number: "37.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Vakði mik Atli,
				<br />
				en ek vera þóttumk
				<br />
				full ills hugar
				<br />
				at frændr dauða.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Разбудил меня Атли —<br />
				в тревоге была я,
				<br />
				предчувствуя смерть
				<br />
				родичей милых.
			</span>
		),
	},
	{
		id: 39,
		number: "38.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Atli kvað:
				<br />
				«Svá mik nýliga
				<br />
				nornir vekja,» —<br />
				vílsinnis spá
				<br />
				vildi, at ek réða, —<br />
				«hugða ek þik, Guðrún
				<br />
				Gjúka dóttir,
				<br />
				læblöndnum hjör
				<br />
				leggja mik í gögnum.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Атли сказал:
				<br />
				«Норны меня
				<br />
				пробудили недавно,
				<br />
				зловещую мне
				<br />
				дали загадку:
				<br />
				мне снилось, ты, Гудрун,
				<br />
				дочь Гьюки, вонзила
				<br />
				отравленный меч
				<br />в тело мое».
			</span>
		),
	},
	{
		id: 40,
		number: "39.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Guðrún kvað:
				<br />
				«Þat er fyr eldi,
				<br />
				er éarn dreyma,
				<br />
				fyr dul ok vil
				<br />
				drósar reiði;
				<br />
				mun ek þik við bölvi
				<br />
				brenna ganga,
				<br />
				líkna ok lækna,
				<br />
				þótt mér leiðir séir.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гудрун сказала:
				<br />
				«Сон про железо
				<br />
				огонь предвещает, —<br />
				женщины гнев —<br />
				желанья пустые:
				<br />
				язву я выжгу
				<br />
				и вылечу хворь,
				<br />
				хоть бы тебя
				<br />я ненавидела».
			</span>
		),
	},
	{
		id: 41,
		number: "40.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Atli kvað:
				<br />
				«Hugða ek hér í túni
				<br />
				teina fallna,
				<br />
				þá er ek vildak
				<br />
				vaxna láta,
				<br />
				rifnir með rótum,
				<br />
				roðnir í blóði,
				<br />
				bornir á bekki,
				<br />
				beðit mik at tyggva.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Атли сказал:
				<br />
				«Снилось еще,
				<br />
				что здесь во дворе
				<br />
				упали побеги, —<br />
				их посадил я, —<br />
				вырваны с корнем,
				<br />
				облиты кровью,
				<br />
				лежат на столе, —<br />я должен жевать их.
			</span>
		),
	},
	{
		id: 42,
		number: "41.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Hugða ek mér af hendi
				<br />
				hauka fljúga
				<br />
				bráðalausa
				<br />
				bölranna til;
				<br />
				hjörtu hugða ek þeira
				<br />
				við hunang tuggin,
				<br />
				sorgmóðs sefa,
				<br />
				sollin blóði.
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Снилось, что пущены
				<br />
				соколы мною,
				<br />
				голодные птицы,
				<br />
				навстречу погибели;
				<br />
				как будто сердца их,
				<br />
				набухшие кровью,
				<br />
				в горе смешал я<br />с медом и съел их.
			</span>
		),
	},
	{
		id: 43,
		number: "42.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Hugða ek mér af hendi
				<br />
				hvelpa losna,
				<br />
				glaums andvana,
				<br />
				gylli báðir;
				<br />
				hold hugða ek þeira
				<br />
				at hræum orðit,
				<br />
				nauðigr nái
				<br />
				nýta ek skyldak.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Снилось, что пущены
				<br />
				мною щенята,
				<br />
				два их, и воют
				<br />
				оба уныло;
				<br />
				снилось, что падалью
				<br />
				стало их мясо,
				<br />
				что его пожирать
				<br />
				принуждают меня».
			</span>
		),
	},
	{
		id: 44,
		number: "43.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Guðrún kvað:
				<br />
				«Þar munu seggir
				<br />
				of sæing dæma
				<br />
				ok hvítinga
				<br />
				höfði næma;
				<br />
				þeir munu feigir
				<br />
				fára nátta
				<br />
				fyrir dag litlu,
				<br />
				drótt of bergja.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Гудрун сказала:
				<br />
				«То воины будут
				<br />
				улов обсуждать,
				<br />
				у белых рыб
				<br />
				отрывая головы:
				<br />
				дохлыми станут
				<br />
				в недолгое время,
				<br />
				перед рассветом
				<br />
				люди съедят их.
			</span>
		),
	},
	{
		id: 45,
		number: "44.",
		class: "flex justify-center m-2",

		contentON: (
			<span className="flex w-40 sm:w-60">
				Atli kvað:
				<br />
				«Lægja ek síðan,
				<br />
				né ek sofa vildak,
				<br />
				þrágjarn í kör;
				<br />
				þat man ek görva.»
			</span>
		),

		contentRU: (
			<span className="flex w-40 sm:w-60">
				Не легла, не спалось мне,
				<br />
				жаждавшей мести,
				<br />
				на ложе моем —<br />
				ясно я помню…»
			</span>
		),
	},
];

export { secondsongofgudrun };
