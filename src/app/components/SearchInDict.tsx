...
export default function SearchInDict() {
	...
	return (
		<div className="w-full mx-auto p-4">
			<h1 className="text-2xl sea-color font-bold mb-4">{t("Title")}</h1>
			<input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("Placeholder")} className="w-full p-2 border rounded-md mb-4" />
			{query === "" ? (
				<p className="text-gray-500 text-center"></p>
			) : results.length === 0 ? (
				<p className="text-gray-500 text-center">{t("Negative")}</p>
			) : (
				<InfiniteScroll
					...
					loader={<p className="text-gray-500 text-center p-2">{t("Loader")}</p>} // Индикатор загрузки
					endMessage={<p className="text-gray-500 text-center p-2">{t("EndMessage")}</p>} // Сообщение при завершении
				>
