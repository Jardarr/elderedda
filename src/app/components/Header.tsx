	...
	return (
		<header className="flex w-full justify-between bg-white/20 dark:bg-gradient-to-r from-sky-950/30 via-black/30 to-sky-950/30 backdrop-blur-lg shadow-lg absolute top-0 right-0 left-0 z-50 px-4 md:px-24">
			<div className="flex items-center">
				...
				<span className="font-Goth uppercase w-10 sm:w-auto text-xs/3 sm:text-xl">GJALLARBRU</span>
			</div>
			<nav className="flex items-center sea-color dark:text-neutral-300 md:my-4 lg:my-0">
				<div className="hidden md:flex md:flex-col lg:flex-row">
					{headerLinks.map((link, index) => (
						<Link
							...
							title={link.aria}
							translate="no"
							className="px-2 hover:text-sky-500 dark:hover:text-neutral-300 text-xs"
							href={link.href}>
							{link.text}
						</Link>
					))}
