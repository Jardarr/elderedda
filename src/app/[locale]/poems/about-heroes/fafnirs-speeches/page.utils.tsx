const fafnirsspeeches = [
    {
        id: 1,
        class: "m-7",

        contentON: (
            <span>
                <p className="mb-2">
                    <span className="font-Eileen text-3xl/4">
                        S
                    </span>
                    igurðr ok Reginn fóru upp á Gnitaheiði ok hittu þar slóð Fáfnis, þá er hann skreið til vatns. Þar gerði Sigurðr gröf mikla á veginum, ok gekk Sigurðr þar í. En
                    er Fáfnir skreið af gullinu, blés hann eitri, ok hraut þat fyrir ofan höfuð Sigurði. En er Fáfnir skreið yfir gröfina, þá lagði Sigurðr hann með sverði til
                    hjarta. Fáfnir hristi sik ok barði höfði ok sporði. Sigurðr hljóp ór gröfinni, ok sá þá hvárr annan. Fáfnir kvað:
                </p>
            </span>
        ),

        contentRU: (
            <span>
                <p>
                    Сигурд и Регин отправились на Гнитахейд и нашли там след Фафнира, который он оставил, когда полз к водопою. Сигурд вырыл большую яму возле следа и засел в ней.
                    И когда Фафнир пополз от сокровища, он изрыгал яд, и яд падал на голову Сигурда. И когда Фафнир проползал над ямой, Сигурд вонзил ему в сердце меч. Фафнир
                    затрясся и стал бить головой и хвостом. Сигурд выскочил из ямы, и они увидели друг друга. Фафнир сказал:
                </p>
            </span>
        ),
    },
    {
        id: 2,
        number: "1.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Sveinn ok sveinn,
                <br />
                hverjum ertu svein of borinn?
                <br />
                Hverra ertu manna mögr,
                <br />
                er þú á Fáfni rautt
                <br />
                þinn inn frána mæki?
                <br />
                Stöndumk til hjarta hjörr.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                «Юнец, юнец!
                <br />
                Кем ты рожден?
                <br />
                Чей сын ты, ответь?
                <br />
                О Фафнира ты
                <br />
                свой меч окровавил;
                <br />в сердце стоит он!»
            </span>
        ),
    },
    {
        id: 3,
        class: "m-7",

        contentON: (
            <span>
                <p className="mb-2">
                    Sigurðr dulði nafn síns, fyrir því at þat var trúa þeira í forneskju, at orð feigs manns mætti mikit, ef hann bölvaði óvin sínum með nafni. Hann kvað:
                </p>
            </span>
        ),

        contentRU: (
            <span>
                <p>
                    Сигурд скрыл свое имя, потому что в древние времена верили, что слова умирающего могущественны, если он проклинает своего недруга, называя его по имени. Он
                    сказал:
                </p>
            </span>
        ),
    },
    {
        id: 4,
        number: "2.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Göfugt dýr ek heiti,
                <br />
                en ek gengit hefk
                <br />
                inn móðurlausi mögr,
                <br />
                föður ek ákk-a
                <br />
                sem fira synir;
                <br />æ geng ek einn saman.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                «Я зверь благородный,
                <br />
                был я всю жизнь
                <br />
                сыном без матери;
                <br />
                нет и отца,
                <br />
                как у людей,
                <br />
                всегда одинок я».
            </span>
        ),
    },
    {
        id: 5,
        number: "3.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Veiztu, ef föður né átt-at
                <br />
                sem fira synir,
                <br />
                af hverju vastu undri alinn?»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Коль нету отца,
                <br />
                как у людей,
                <br />
                чем же рожден ты?»
            </span>
        ),
    },
    {
        id: 6,
        number: "4.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Ætterni mitt
                <br />
                kveð ek þér ókunnigt vera
                <br />
                ok mik sjalfan it sama;
                <br />
                Sigurðr ek heiti,
                <br />
                Sigmundr hét minn faðir,
                <br />
                er hefk þik vápnum vegit.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Род мой тебе
                <br />
                еще не ведом,
                <br />
                и сам я тоже:
                <br />
                Сигурд зовусь —<br />
                Сигмунд отец мой,
                <br />
                мной ты сражен».
            </span>
        ),
    },
    {
        id: 7,
        number: "5.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Hverr þik hvatti?
                <br />
                Hví hvetjask lézt
                <br />
                mínu fjörvi at fara?
                <br />
                Inn fráneygi sveinn,
                <br />
                þú áttir föður bitran;
                <br />
                óbornum skjór á skeið.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Кто тебя подстрекнул,
                <br />
                почему ты решился
                <br />
                жизнь отнять у меня?
                <br />
                Взор твой сверкает,
                <br />
                сын храбреца,
                <br />
                ты с детства был храбрым!»
            </span>
        ),
    },
    {
        id: 8,
        number: "6.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Hugr mik hvatti,
                <br />
                hendr mér fulltýðu
                <br />
                ok minn inn hvassi hjörr;
                <br />
                fár er hvatr,
                <br />
                er hröðask tekr,
                <br />
                ef í barnæsku er blauðr.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Смелость вела,
                <br />
                помогали руки
                <br />
                и крепкий клинок мой;
                <br />
                храбрым не станет
                <br />
                стареющий воин,
                <br />
                если в детстве был трусом».
            </span>
        ),
    },
    {
        id: 9,
        number: "7.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Veit ek, ef þú vaxa næðir
                <br />
                fyr þinna vina brjósti,
                <br />
                sæi maðr þik vreiðan vega;
                <br />
                nú ertu haftr
                <br />
                ok hernuminn;
                <br />æ kveða bandingja bifask.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Знаю: если б возрос
                <br />
                на груди у друзей, —<br />
                разил бы рьяно;
                <br />
                но, в неволе рожденный,
                <br />
                стал ты рабом
                <br />и робеешь, как раб».
            </span>
        ),
    },
    {
        id: 10,
        number: "8.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Því bregðr þú mér, Fáfnir,
                <br />
                at til fjarri séak
                <br />
                mínum feðrmunum;
                <br />
                eigi em ek haftr,
                <br />
                þótt ek væra hernumi;
                <br />
                þú fannt, at ek laus lifi.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «К чему твой попрек,
                <br />
                что я далеко
                <br />
                от наследья отца!
                <br />
                Нет, я не раб,
                <br />
                хоть пленником был;
                <br />я свободен, ты видишь!»
            </span>
        ),
    },
    {
        id: 11,
        number: "9.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Heiftyrði ein
                <br />
                telr þú þér í hvívetna,
                <br />
                en ek þér satt eitt segik:
                <br />
                It gjalla gull
                <br />
                ok it glóðrauða fé,
                <br />
                þér verða þeir baugar at bana.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Слышишь ты всюду
                <br />
                слово вражды,
                <br />
                но прав я, поверь:
                <br />
                золото звонкое,
                <br />
                клад огнекрасный,
                <br />
                погубит тебя!»
            </span>
        ),
    },
    {
        id: 12,
        number: "10.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Féi ráða
                <br />
                vill fyrða hverr
                <br />
                æ til ins eina dags;
                <br />
                því at einu sinni
                <br />
                skal alda hverr
                <br />
                fara til heljar heðan.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Богатством владеть
                <br />
                всем суждено
                <br />
                до какого-то дня,
                <br />
                ибо для всех
                <br />
                время настанет
                <br />в могилу сойти».
            </span>
        ),
    },
    {
        id: 13,
        number: "11.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Norna dóm
                <br />
                þú munt fyr nesjum hafa
                <br />
                ok ósvinns apa;
                <br />
                í vatni þú drukknar,
                <br />
                ef í vindi rær;
                <br />
                allt er feigs forað.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Норн приговор
                <br />
                у мыса узнаешь
                <br />
                и жребий глупца;
                <br />
                в бурю ты станешь
                <br />
                грести осторожно,
                <br />и все ж ты потонешь».
            </span>
        ),
    },
    {
        id: 14,
        number: "12.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Segðu mér, Fáfnir,
                <br />
                alls þik fróðan kveða
                <br />
                ok vel margt vita,
                <br />
                hverjar ro þær nornir,
                <br />
                er nauðgönglar ro
                <br />
                ok kjósa mæðr frá mögum.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Фафнир, скажи мне,
                <br />
                ты мудр, я слышал,
                <br />
                и многое знаешь:
                <br />
                кто эти норны,
                <br />
                что могут прийти
                <br />к женам рожающим?»
            </span>
        ),
    },
    {
        id: 15,
        number: "13.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Sundrbornar mjök
                <br />
                segi ek nornir vera,
                <br />
                eigu-t þær ætt saman;
                <br />
                sumar eru áskunngar,
                <br />
                sumar alfkunngar,
                <br />
                sumar dætr Dvalins.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Различны рожденьем
                <br />
                норны, я знаю, —<br />
                их род не единый:
                <br />
                одни от асов,
                <br />
                от альвов иные,
                <br />
                другие от Двалина».
            </span>
        ),
    },
    {
        id: 16,
        number: "14.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Segðu mér þat, Fáfnir,
                <br />
                alls þik fróðan kveða
                <br />
                ok vel margt vita,
                <br />
                hvé sá holmr heitir,
                <br />
                er blanda hjörlegi
                <br />
                Surtr ok æsir saman.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Фафнир, скажи мне,
                <br />
                ты мудр, я слышал,
                <br />
                и многое знаешь:
                <br />
                как остров зовется,
                <br />
                где кровь смешают
                <br />
                асы и Сурт?»
            </span>
        ),
    },
    {
        id: 17,
        number: "15.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Óskópnir hann heitir,
                <br />
                en þar öll skulu
                <br />
                geirum leika goð;
                <br />
                Bilröst brotnar,
                <br />
                er þeir á brú fara,
                <br />
                ok svima í móðu marir.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Оскопнир — остров,
                <br />
                богам суждено там
                <br />
                копьями тешиться;
                <br />
                Бильрест рухнет,
                <br />
                вплавь будут кони
                <br />
                прочь уносить их.
            </span>
        ),
    },
    {
        id: 18,
        number: "16.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Ægishjalm
                <br />
                bar ek of alda sonum,
                <br />
                meðan ek of menjum lák;
                <br />
                einn rammari
                <br />
                hugðumk öllum vera,
                <br />
                fannk-a ek svá marga mögu.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Шлем-страшило
                <br />
                носил я всегда,
                <br />
                на золоте лежа;
                <br />
                всех сильнее
                <br />
                себя я считал,
                <br />с кем бы ни встретился».
            </span>
        ),
    },
    {
        id: 19,
        number: "17.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Ægishjalmr
                <br />
                bergr einungi,
                <br />
                hvar skulu vreiðir vega;
                <br />
                þá þat finnr,
                <br />
                er með fleirum kemr,
                <br />
                at engi er einna hvatastr.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Шлем-страшило
                <br />
                не защитит
                <br />
                в схватке смелых;
                <br />
                в том убедится
                <br />
                бившийся часто,
                <br />
                что есть и сильнейшие».
            </span>
        ),
    },
    {
        id: 20,
        number: "18.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Eitri ek fnæsta,
                <br />
                er ek á arfi lá
                <br />
                miklum míns föður.»
                <br />
                *********
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Яд изрыгал я,
                <br />
                когда лежал
                <br />
                на наследстве отцовом».
            </span>
        ),
    },
    {
        id: 21,
        number: "19.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Inn fráni ormr,
                <br />
                þú gerðir fræs mikla
                <br />
                ok galzt harðan hug;
                <br />
                heift at meiri
                <br />
                verðr hölða sonum,
                <br />
                at þann hjalm hafi.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Змей могучий,
                <br />
                шипел ты громко
                <br />
                и храбрым ты был;
                <br />
                оттого сильнее
                <br />
                людей ненавидел,
                <br />
                что шлемом владел ты».
            </span>
        ),
    },
    {
        id: 22,
        number: "20.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Ræð ek þér nú, Sigurðr,
                <br />
                en þú ráð nemir,
                <br />
                ok ríð heim heðan;
                <br />
                it gjalla gull
                <br />
                ok it glóðrauða fé,
                <br />
                þér verða þeir baugar at bana.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Дам тебе, Сигурд,
                <br />
                совет, — прими его:
                <br />
                вспять возвратись ты!
                <br />
                Золото звонкое,
                <br />
                клад огнекрасный,
                <br />
                погубит тебя!»
            </span>
        ),
    },
    {
        id: 23,
        number: "21.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Ráð er þér ráðit,
                <br />
                en ek ríða mun
                <br />
                til þess gulls, er í lyngvi liggr,
                <br />
                en þú, Fáfnir,
                <br />
                ligg í fjörbrotum,
                <br />
                þar er þik hel hafi.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «С тобой покончено,
                <br />
                я ж поспешу
                <br />
                к золоту в вереске;
                <br />
                Фафнир, валяйся
                <br />
                средь жизни обломков, —<br />
                Хель заберет тебя!»
            </span>
        ),
    },
    {
        id: 24,
        number: "22.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Fáfnir kvað:
                <br />
                «Reginn mik réð,
                <br />
                hann þik ráða mun,
                <br />
                hann mun okkr verða báðum at bana;
                <br />
                fjör sitt láta,
                <br />
                hygg ek, at Fáfnir myni;
                <br />
                þitt varð nú meira megin.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Фафнир сказал:
                <br />
                «Предан я Регином,
                <br />
                предаст и тебя он,
                <br />
                погибнем мы оба;
                <br />
                сдается мне, Фафнир
                <br />
                с жизнью простится, —<br />
                ты, Сигурд, сильнее».
            </span>
        ),
    },
    {
        id: 25,
        class: "m-7",

        contentON: (
            <span>
                <p className="mb-2">Reginn var á brott horfinn, meðan Sigurðr vá Fáfni, ok kom þá aftr, er Sigurðr strauk blóð af sverðinu. Reginn kvað:</p>
            </span>
        ),

        contentRU: (
            <span>
                <p>Регина не было, когда Сигурд убивал Фафнира. Он вернулся, когда Сигурд вытирал кровь с меча. Регин сказал:</p>
            </span>
        ),
    },
    {
        id: 26,
        number: "23.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Heill þú nú, Sigurðr,
                <br />
                nú hefir þú sigr vegit
                <br />
                ok Fáfni of farit;
                <br />
                manna þeira,
                <br />
                er mold troða,
                <br />
                þik kveð ek óblauðastan alinn.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                «Привет тебе, Сигурд,
                <br />
                в бою победил ты,
                <br />
                с Фафниром справясь;
                <br />
                из всех людей,
                <br />
                попирающих землю,
                <br />
                ты самый смелый».
            </span>
        ),
    },
    {
        id: 27,
        number: "24.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Þat er óvíst at vita,
                <br />
                þá er komum allir saman,
                <br />
                sigtíva synir,
                <br />
                hverr óblauðastr er alinn;
                <br />
                margr er sá hvatr,
                <br />
                er hjör né rýðr
                <br />
                annars brjóstum í.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Как указать,
                <br />
                когда соберутся
                <br />
                богов сыновья,
                <br />
                кто самый смелый?
                <br />
                Многие смелы,
                <br />
                клинка не омыв
                <br />
                во вражьей крови».
            </span>
        ),
    },
    {
        id: 28,
        number: "25.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Reginn kvað:
                <br />
                «Glaðr ertu nú, Sigurðr,
                <br />
                ok gagni feginn,
                <br />
                er þú þerrir Gram á grasi;
                <br />
                bróður minn
                <br />
                hefr þú benjaðan,
                <br />
                ok vald ek þó sjalfr sumu.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Регин сказал:
                <br />
                «Рад ты, Сигурд,
                <br />
                с Грама кровь
                <br />
                о траву отирая;
                <br />
                брат мой родной
                <br />
                тобою убит,
                <br />в том виновен я тоже».
            </span>
        ),
    },
    {
        id: 29,
        number: "26.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Þú því rétt,
                <br />
                er ek ríða skyldak
                <br />
                hélug fjöll hinig;
                <br />
                féi ok fjörvi
                <br />
                réði sá inn fráni ormr,
                <br />
                nema þú frýðir mér hvats hugar.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Виновен ты в том,
                <br />
                что сюда я приехал
                <br />
                по склонам священным;
                <br />
                богатством и жизнью
                <br />
                змей бы владел, —<br />
                ты к битве понудил».
            </span>
        ),
    },
    {
        id: 30,
        class: "m-7",

        contentON: (
            <span>
                <p className="mb-2">Þá gekk Reginn at Fáfni ok skar hjarta ór hánum með sverði, er Riðill heitir, ok þá drakk hann blóð ór undinni eftir. Reginn kvað:</p>
            </span>
        ),

        contentRU: (
            <span>
                <p>Тогда Регин подошел к Фафниру и вырезал у него сердце мечом, который называется Ридиль. Затем он стал пить кровь из раны.</p>
            </span>
        ),
    },
    {
        id: 31,
        number: "27.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Sittu nú, Sigurðr,
                <br />
                en ek mun sofa ganga,
                <br />
                ok halt Fáfnis hjarta við funa;
                <br />
                eisköld ek vil
                <br />
                etin láta
                <br />
                eftir þenna dreyra drykk.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Регин сказал:
                <br />
                «Спать я пойду,
                <br />
                ты ж подержи
                <br />
                в пламени сердце!
                <br />
                Его я потом
                <br />
                отведать хочу
                <br />с напитком кровавым».
            </span>
        ),
    },
    {
        id: 32,
        number: "28.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Fjarri þú gekkt,
                <br />
                meðan ek á Fáfni rauðk
                <br />
                minn inn hvassa hjör;
                <br />
                afli mínu
                <br />
                atta ek við orms megin,
                <br />
                meðan þú í lyngvi látt.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Был ты далеко,
                <br />
                когда обагрял я<br />
                о Фафнира меч;
                <br />
                силами я<br />
                со змеем померился,
                <br />
                пока отдыхал ты».
            </span>
        ),
    },
    {
        id: 33,
        number: "29.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Reginn kvað:
                <br />
                «Lengi liggja
                <br />
                létir þú lyngvi í<br />
                þann inn aldna jötun,
                <br />
                ef þú sverðs né nytir,
                <br />
                þess er ek sjalfr gerða,
                <br />
                ok þíns ins hvassa hjörs.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Регин сказал:
                <br />
                «Ты дал бы лежать
                <br />
                долго в траве
                <br />
                старику исполину,
                <br />
                если за острый
                <br />
                не взялся бы меч, —<br />
                но ведь я его выковал».
            </span>
        ),
    },
    {
        id: 34,
        number: "30.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Hugr er betri
                <br />
                en sé hjörs megin,
                <br />
                hvars vreiðir skulu vega,
                <br />
                því at hvatan mann
                <br />
                ek sé harðliga vega
                <br />
                með slævu sverði sigr.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «Смелость лучше
                <br />
                силы меча
                <br />
                в битве героев, —<br />
                доблестный муж
                <br />
                одержит победу
                <br />
                мечом ненаточенным.
            </span>
        ),
    },
    {
        id: 35,
        number: "31.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Hvötum er betra
                <br />
                en sé óhvötum
                <br />
                í hildileik hafask;
                <br />
                glöðum er betra
                <br />
                en sé glúpnanda,
                <br />
                hvat sem at hendi kemr.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Смелому лучше,
                <br />
                чем трусу, придется
                <br />
                в играх валькирий;
                <br />
                лучше храбрец,
                <br />
                чем разиня испуганный,
                <br />
                что б ни случилось».
            </span>
        ),
    },
    {
        id: 36,
        class: "m-7",

        contentON: (
            <span>
                <p className="mb-2">
                    Sigurðr tók Fáfnis hjarta ok steikði á teini. Er hann hugði, at fullsteikt væri ok freyddi sveitinn ór hjartanu, þá tók hann á fingri sínum ok skynjaði, hvárt
                    fullsteikt væri. Hann brann ok brá fingrinum í munn sér. En er hjartablóð Fáfnis kom á tungu hánum, ok skilði hann fugls rödd. Hann heyrði, at igður klökuðu á
                    hrísinu. Igðan kvað:
                </p>
            </span>
        ),

        contentRU: (
            <span>
                <p>
                    Сигурд взял сердце Фафнира и стал поджаривать его на палочке. Когда он решил, что оно изжарилось, и кровь из сердца запенилась, он дотронулся до него пальцем,
                    чтобы узнать, готово ли оно. Он обжегся и поднес палец ко рту. Но когда кровь из сердца Фафнира попала ему на язык, он стал понимать птичью речь. Он услышал,
                    как щебечут синицы в кустах. Синица сказала:
                </p>
            </span>
        ),
    },
    {
        id: 37,
        number: "32.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Þar sitr Sigurðr
                <br />
                sveita stokkinn,
                <br />
                Fáfnis hjarta
                <br />
                við funa steikir;
                <br />
                spakr þætti mér
                <br />
                spillir bauga,
                <br />
                ef hann fjörsega
                <br />
                fránan æti.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                «Вот конунг Сигурд,
                <br />
                обрызганный кровью,
                <br />
                Фафнира сердце
                <br />
                хочет поджарить;
                <br />
                мудрым сочла бы
                <br />
                дарящего кольца,
                <br />
                если б он съел
                <br />
                сердце блестящее».
            </span>
        ),
    },
    {
        id: 38,
        number: "33.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Önnur kvað:
                <br />
                «Þar liggr Reginn,
                <br />
                ræðr um við sik,
                <br />
                vill tæla mög,
                <br />
                þann er trúir hánum,
                <br />
                berr af reiði
                <br />
                röng orð saman,
                <br />
                vill bölvasmiðr
                <br />
                bróður hefna.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Вторая сказала:
                <br />
                «Вот Регин лежит,
                <br />
                он злое задумал,
                <br />
                обманет он князя,
                <br />
                а тот ему верит;
                <br />
                в гневе слагает
                <br />
                злые слова,
                <br />
                за брата отмстит
                <br />
                злобу кующий».
            </span>
        ),
    },
    {
        id: 39,
        number: "34.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                In þriðja kvað:
                <br />
                «Höfði skemmra
                <br />
                láti hann inn hára þul
                <br />
                fara til heljar heðan;
                <br />
                öllu gulli
                <br />
                þá kná hann einn ráða,
                <br />
                fjölð því er und Fáfni lá.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Третья сказала:
                <br />
                «Тула седого
                <br />
                пусть обезглавит, —<br />
                в Хель ему место!
                <br />
                Сокровищем всем,
                <br />
                что Фафнир стерег,
                <br />
                один владел бы».
            </span>
        ),
    },
    {
        id: 40,
        number: "35.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                In fjórða kvað:
                <br />
                Horskr þætti mér,
                <br />
                ef hafa kynni
                <br />
                ástráð mikit
                <br />
                yðvar systra,
                <br />
                hygði hann of sik
                <br />
                ok hugin gleddi;
                <br />
                þar er mér ulfs ván,
                <br />
                er ek eyru sék.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Четвертая сказала:
                <br />
                «Умным сочла бы,
                <br />
                когда б он послушался
                <br />
                наших, сестры,
                <br />
                добрых советов;
                <br />
                о себе бы радел
                <br />
                и радовал ворона;
                <br />
                волка узнаешь
                <br />
                по волчьим ушам».
            </span>
        ),
    },
    {
        id: 41,
        number: "36.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                In fimmta kvað:
                <br />
                «Er-at svá horskr
                <br />
                hildimeiðr
                <br />
                sem ek hers jaðar
                <br />
                hyggja myndak,
                <br />
                ef hann bróður lætr
                <br />
                á brot komask,
                <br />
                en hann öðrum hefir
                <br />
                aldrs of synjat.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Пятая сказала:
                <br />
                «Не будет мудрым
                <br />
                ясень сраженья,
                <br />
                каким я войска
                <br />
                считала вершину,
                <br />
                если позволит
                <br />
                уйти человеку,
                <br />
                брат которого
                <br />
                был им убит».
            </span>
        ),
    },
    {
        id: 42,
        number: "37.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                In sétta kvað:
                <br />
                «Mjök er ósviðr,
                <br />
                ef hann enn sparir
                <br />
                fjánda inn folkskáa,
                <br />
                þar er Reginn liggr,
                <br />
                er hann ráðinn hefr,
                <br />
                kann-at hann við slíku at séa.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Шестая сказала:
                <br />
                «Глупо поступит,
                <br />
                когда пощадит
                <br />
                низкого недруга;
                <br />
                Регин лежит здесь,
                <br />
                предавший его,
                <br />
                зло он задумал».
            </span>
        ),
    },
    {
        id: 43,
        number: "38.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                In sjaunda kvað:
                <br />
                «Höfði skemmra láti hann
                <br />
                þann inn hrímkalda jötun
                <br />
                ok af baugum búa;
                <br />
                þá mun hann fjár þess,
                <br />
                er Fáfnir réð,
                <br />
                einvaldi vera.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Седьмая сказала:
                <br />
                «Пусть великану
                <br />
                он голову срубит
                <br />
                и кольца отнимет;
                <br />
                тогда завладеет
                <br />
                золотом всем,
                <br />
                что у Фафнира было».
            </span>
        ),
    },
    {
        id: 44,
        number: "39.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                Sigurðr kvað:
                <br />
                «Verða-t svá rík sköp,
                <br />
                at Reginn skyli
                <br />
                mitt banorð bera;
                <br />
                því at þeir báðir bræðr
                <br />
                skulu bráðliga
                <br />
                fara til heljar heðan.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сигурд сказал:
                <br />
                «У судьбы не возьмет
                <br />
                Регин той силы,
                <br />
                что смерть мне сулила б,
                <br />
                вдвоем должны
                <br />
                в Хель поспешать
                <br />
                братья отсюда».
            </span>
        ),
    },
    {
        id: 45,
        class: "m-7",

        contentON: (
            <span>
                <p className="mb-2">Sigurðr hjó höfuð af Regin, ok þá át han Fáfnis hjarta ok drakk blóð þeira beggja, Regins ok Fáfnis. Þá heyrði Sigurðr, hvar igður mæltu:</p>
            </span>
        ),

        contentRU: (
            <span>
                <p>Сигурд отрубил голову Регину. Затем он отведал сердца Фафнира и отпил крови обоих — Регина и Фафнира. Тогда Сигурд услышал, как синицы говорили:</p>
            </span>
        ),
    },
    {
        id: 46,
        number: "40.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Bitt þú, Sigurðr,
                <br />
                bauga rauða;
                <br />
                er-a konungligt
                <br />
                kvíða mörgu.
                <br />
                Mey veit ek eina
                <br />
                miklu fegrsta,
                <br />
                gulli gædda,
                <br />
                ef þú geta mættir.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                «Связывай кольца
                <br />
                красные, Сигурд,
                <br />
                долго тревожиться
                <br />
                конунг не должен!
                <br />
                Знаю, есть дева —<br />
                золотом убрана,
                <br />
                прекрасна лицом —<br />
                твоей быть могла бы.
            </span>
        ),
    },
    {
        id: 47,
        number: "41.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Liggja til Gjúka
                <br />
                grænar brautir,
                <br />
                fram vísa sköp
                <br />
                folklíðöndum;
                <br />
                þar hefir dýrr konungr
                <br />
                dóttur alna;
                <br />
                þá muntu, Sigurðr,
                <br />
                mundi kaupa.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                К Гьюки ведут
                <br />
                зеленые тропы,
                <br />
                страннику путь
                <br />
                укажет судьба!
                <br />
                Конунг достойный
                <br />
                дочь взрастил там,
                <br />
                Сигурд, за деву
                <br />
                ты вено заплатишь.
            </span>
        ),
    },
    {
        id: 48,
        number: "42.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Salr er á háu
                <br />
                Hindarfjalli,
                <br />
                allr er hann útan
                <br />
                eldi sveipinn,
                <br />
                þann hafa horskir
                <br />
                halir of görvan
                <br />
                ór ódökkum
                <br />
                Ógnar ljóma.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Высокий чертог
                <br />
                на вершине Хиндарфьялль,
                <br />
                весь опоясан
                <br />
                снаружи огнем;
                <br />
                мудрые люди
                <br />
                его воздвигли
                <br />
                из пламени вод,
                <br />
                тьму озарившего.
            </span>
        ),
    },
    {
        id: 49,
        number: "43.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Veit ek á fjalli
                <br />
                folkvitr sofa
                <br />
                ok leikr yfir
                <br />
                lindar váði;
                <br />
                Yggr stakk þorni,
                <br />
                aðra felldi
                <br />
                hör-Gefn hali
                <br />
                en hafa vildi.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Знаю — валькирия
                <br />
                спит на вершине,
                <br />
                ясеня гибель
                <br />
                играет над нею;
                <br />
                усыпил ее Один,
                <br />
                шипом уколов, —<br />
                не того сгубила,
                <br />
                кто был ей указан.
            </span>
        ),
    },
    {
        id: 50,
        number: "44.",
        class: "flex justify-center m-2",

        contentON: (
            <span className="flex w-40 sm:w-60">
                «Knáttu, mögr, séa
                <br />
                mey und hjalmi,
                <br />
                þá er frá vígi
                <br />
                Vingskorni reið;
                <br />
                má-at Sigrdrífar
                <br />
                svefni bregða
                <br />
                skjöldunga niðr
                <br />
                fyr sköpum norna.»
            </span>
        ),

        contentRU: (
            <span className="flex w-40 sm:w-60">
                Сможешь увидеть
                <br />
                деву под шлемом;
                <br />
                вынес из битвы
                <br />
                Вингскорнир деву;
                <br />
                не в силах Сигрдрива
                <br />
                сон побороть,
                <br />
                конунгов отпрыск, —<br />
                так норна велела».
            </span>
        ),
    },
    {
        id: 51,
        class: "m-7",

        contentON: (
            <span>
                <p className="mb-2">
                    Sigurðr reið eftir slóð Fáfnis til bælis hans ok fann þat opit ok hurðir af járni ok gætti. Af járni váru ok allir timbrstokkar í húsinu, en grafit í jörð niðr.
                    Þar fann Sigurðr stórmikit gull ok fylldi þar tvær kistur. Þar tók hann ægishjálm ok gullbrynju ok sverðit Hrotta ok marga dýrgripi ok klyfjaði þar með Grana,
                    en hestrinn vildi eigi fram ganga, fyrr en Sigurðr steig á bak hánum.
                </p>
            </span>
        ),

        contentRU: (
            <span>
                <p>
                    Сигурд поехал по следу Фафнира в его логово и нашел его открытым, и двери были железными, и дверная рама тоже. Железными были также все балки в доме, и дом был
                    закопан в землю. Там Сигурд нашел очень много золота и наполнил им два сундука. Там он взял шлем-страшило, золотую кольчугу, меч Хротти и много сокровищ и
                    нагрузил всем этим Грани. Но конь не хотел идти, пока Сигурд не сел на него.
                </p>
            </span>
        ),
    },
];

export { fafnirsspeeches };
